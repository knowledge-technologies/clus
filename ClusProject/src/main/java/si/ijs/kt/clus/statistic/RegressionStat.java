...
public class RegressionStat extends RegressionStatBase implements ComponentStatistic {
	...
	// daniela
	// matejp: removed static in INITIALIZE_PARTIAL_SUM, and made every field in
	// this daniela block private
	...
	/**
	 * Clone this statistic by taking the given weight into account. This is used
	 * for example to get the weighted prediction of default rule.
	 */
	@Override
	...
	public double getMean(int i) {
		// added by Jurica (if we call getMean() and m_SumWeights or m_SumValues are not
		// initialized (e.g. after cloneSimple) it chrashes)
		if (m_SumValues == null || m_SumWeights == null) {
			return m_Means[i];
		}
		// - end added by Jurica

		// If divider zero, i.e., for i-th attribute we have no labeled examples or all
		// missing values
		if (m_SumWeights[i] == 0) {
    ...
    public double getEstimatedSVarS(int i) {
        switch (getSettings().getTree().getMissingClusteringAttrHandling()) {
        	case EstimateFromParentNode:
                if (m_ParentStat == null)
                    return Double.NaN; // the case if for attribute i all examples gave missing values (if there is no
                                       // parent stat, it means we reached the root node)
	...
	/**
	 * Should the variance be calculated, or estimated (for example if all examples
	 * have missing values)
	 * 
	 * @param k_tot
	 *            Total number of examples with non-missing values for i-th
	 *            attribute
	 * @param n_tot
	...
	public boolean shouldEstimate(double k_tot, double n_tot) {
		...
		// return k_tot <= 1 && k_tot != n_tot || n_tot <= MathUtil.C1E_9; //k_tot == 1
		// is allowed only if k_tot==n_tot
	...
	public double getSVarS(int i) {
		...
		// }
		// else {
		// return (k_tot > 1.0) ? ss_tot * (n_tot - 1) / (k_tot - 1) - n_tot * sv_tot /
		// k_tot * sv_tot / k_tot : 0.0;
		// }
	}
	...
	public double getSVarS(double n_tot, double k_tot, double sv_tot, double ss_tot) {
		...
		// return ss_tot * (n_tot - 1) / (k_tot - 1) - n_tot * sv_tot / k_tot * sv_tot /
		// k_tot;
	}
		...
		for (int i = 0; i < m_NbAttrs; i++) {
			...
			if (k_tot == n_tot) {
				...
				if (k_tot <= MathUtil.C1E_9 && m_Training != null) {
					...
					result += ((ss_tot * (n_tot - 1) / (k_tot - 1) - n_tot * sv_tot / k_tot * sv_tot / k_tot)
							* scale.getWeight(m_Attrs[i])) * hmtrWeight;
				}
			}
		}
	...
	public void calcMeanSpatial(double[] means) {
		...
		for (int k = 0; k < m_NbAttrs; k++) {
			ClusAttrType type = schema.getNumericAttrUse(AttributeUseType.Target)[k]; // target
			ClusAttrType xt = schema.getNumericAttrUse(AttributeUseType.GIS)[0]; // x coord
			ClusAttrType yt = schema.getNumericAttrUse(AttributeUseType.GIS)[1]; // y coord
			int NeighCount = 3;
			...
			for (int i = 0; i < N; i++) {
				...
				for (int j = 0; j < N; j++) {
					...
					if (N < NeighCount)
						...
						if (j < NeighCount)
							...
							biggestd = distances[0].distance; // go through the knn list and replace the biggest one if
																// possible
							biggestindex = 0;
				...
				for (int ii = 0; ii < NeighCount; ii++) {
					m_SumValuesSpatial[i] += distances[ii].target; // for this test instance/example this is the
																	// prediction, but I do not know how to assign it
				}
			}
	...
	public double calcPDistance(Integer[] permutation) throws ClusException {
		...
		for (int k = 0; k < schema.getNbTargetAttributes(); k++) {
			for (int i = M; i < N; i++) {
				ClusAttrType type = schema.getNumericAttrUse(AttributeUseType.Target)[k];
				...
				for (int i = M; i < N; i++) {
					...
					for (int j = M; j < N; j++) {
						...
						if ((N - M) < NeighCount)
							...
							if (permutation[j] < NeighCount)
								...
								biggestd = distances[0].distance; // go through the knn list and replace the biggest one
																	// if possible
								biggestindex = 0;
								for (int a = 1; a < NeighCount; a++)
									if (distances[a].distance > biggestd) {
					...
					for (int j = M; j < NN; j++) {
						if (distances[j].distance == 0.0)
							...
							case Modified:
								w[permutation[i]][permutation[j]] = (1
										- (distances[j].distance * distances[j].distance) / (NeighCount * NeighCount))
										* (1 - (distances[j].distance * distances[j].distance)
												/ (NeighCount * NeighCount));
								break; // modified
							case Gaussian:
								w[permutation[i]][permutation[j]] = Math.exp(
										-(distances[j].distance * distances[j].distance) / (NeighCount * NeighCount));
								break; // gausian
							default:
								w[permutation[i]][permutation[j]] = 1;
						...
						upsum[k] += w[permutation[i]][permutation[j]] * (ti - means[k])
								* (distances[j].target - means[k]); // m_distances.get(i*N+j)
																	// [i][permutation[j]]
			...
			if (NeighCount > 0) {
				...
				for (int i = M; i < N; i++) {
					...
					while ((distances.length > j) && (distances[j] != null) && j < NeighCount) {
						...
						upsumR[k] += w[permutation[i]][permutation[j]] * (ti - meansR[k])
								* (distances[j].target - meansR[k]); // m_distances.get(i*N+j)
		...
		for (int i = M; i < N; i++) {
			...
			for (int j = M; j < N; j++) {
				...
				if (i != j) {
					...
					upsum[0][i] += w * (xj[0] - means[0]);
					upsum[1][i] += w * (xj[1] - means[1]);
					W += w;
				} else {
					// upsum[0][i] += xi[0]; upsum[1][i] +=xi[1];W+=1;
					upsum[0][i] += xi[0] - means[0];
					upsum[1][i] += xi[1] - means[1];
					W += 1;
				}
			}
			// upsum0+= (upsum[0][permutation[i]]-means[0]); upsum1+=
			// (upsum[1][permutation[i]]-means[1]);
			WL += W * W;
			...
			for (int j = M; j < N; j++) {
				...
				if (i != j) {
					...
					if (temp != null)
						...
						w = 0;
					upsumR[0][(i - M)] += w * (xjR[0] - meansR[0]);
					upsumR[1][(i - M)] += w * (xjR[1] - meansR[1]);
					WR += w;
				} else {
					upsumR[0][(i - M)] += xiR[0] - meansR[0];
					upsumR[1][(i - M)] += xiR[1] - meansR[1];
					WR += 1;
				}
			}
			WRR += WR * WR;
				...
				for (int j = M; j < NN; j++) {
					if (distances[j].distance == 0.0)
						...
						case Modified:
							w[permutation[i]][permutation[j]] = (1
									- (distances[j].distance * distances[j].distance) / (NeighCount * NeighCount))
									* (1 - (distances[j].distance * distances[j].distance) / (NeighCount * NeighCount));
							break; // modified
						case Gaussian:
							w[permutation[i]][permutation[j]] = Math
									.exp(-(distances[j].distance * distances[j].distance) / (NeighCount * NeighCount));
							break; // gausian
						default:
							w[permutation[i]][permutation[j]] = 1;
							break;
	...
	public double calcCItotalD(Integer[] permutation) throws ClusException {
		...
		for (int k = 0; k < schema.getNbTargetAttributes(); k++) {
			...
			for (int i = M; i < N; i++) {
				for (int j = M; j < N; j++)
					if (i != j && DR[(j - M)] != 0)
						WR[(i - M)] += wr[(i - M)][(j - M)] / Math.sqrt(DR[(j - M)]);
				if (DR[(i - M)] != 0)
					ikkR[k] += WR[(i - M)] / Math.sqrt(DR[(i - M)]);
			}
	...
	public double calcCItotal(Integer[] permutation) throws ClusException {
		...
		double I = (IL + IR) / m_Data.getNbRows(); // System.out.println("Join Moran I: "+I+" eLeft: "+(NR-(N-M))+"
													// eRight: "+(N-M)+" "+IL+" "+IR);
		double scaledI = 1 + I;
		if (Double.isNaN(I)) {
			throw new ClusException("err!");
			...
			if (NeighCount > 0) {
				...
				for (int i = M; i < N; i++) {
					...
					for (int j = M; j < NN; j++) {
						...
						upsum[k] += w[permutation[i]][permutation[j]] * (ti - distances[j].target)
								* (ti - distances[j].target);
						W += w[permutation[i]][permutation[j]];
					}
					downsum[k] += ((ti - means[k]) * (ti - means[k]));
				}
			...
			if (NeighCount > 0) {
				...
				for (int i = M; i < N; i++) {
					...
					while ((distances.length > j) && (distances[j] != null) && j < NeighCount) {
						...
						upsumR[k] += w[permutation[i]][permutation[j]] * (ti - distances[j].target)
								* (ti - distances[j].target);
						WR += w[permutation[i]][permutation[j]];
						j++;
					}
	...
	public double calcEquvalentIwithNeighbourstotal() throws ClusException {
		...
		/*
		 * for (int k = 0; k < schema.getNbTargetAttributes(); k++) {
		 * a[k]=m_PreviousSumW[k]; b[k]=m_PreviousSumWXX[k]; c[k]=m_PreviousSumWX[k];
		 * d[k]=m_PreviousSumX[k]; e[k]=m_PreviousSumX2[k]; int NeighCount =
		 * (int)schema.getSettings().getNumNeightbours(); //30; double[][] w = new
		 * double[N][N]; //matrica double W = 0.0; if (NeighCount>0) { for (int i = M; i
		 * < N; i++) { Distance distances[]=new Distance[NeighCount]; ClusAttrType type
		 * = schema.getNumericAttrUse(AttributeUseType.Target)[k]; DataTuple exi =
		 * m_Data.getTuple(permutation[i]); double ti = type.getNumeric(exi);
		 * ClusAttrType xt = schema.getNumericAttrUse(AttributeUseType.GIS)[0]; //x
		 * coord ClusAttrType yt =
		 * schema.getNumericAttrUse(AttributeUseType.GIS)[1]; //y coord double xi =
		 * xt.getNumeric(exi); double yi = yt.getNumeric(exi); double
		 * biggestd=Double.POSITIVE_INFINITY; int biggestindex=Integer.MAX_VALUE; d[k]
		 * += xi; e[k] += xi*xi; for (int j = M; j <N; j++) { DataTuple exj =
		 * m_Data.getTuple(j); double tj = type.getNumeric(exj); double xj =
		 * xt.getNumeric(exj); double yj = yt.getNumeric(exj); double tempd =
		 * Math.sqrt((xi-xj)*(xi-xj)+(yi-yj)*(yi-yj)); if
		 * (vkupenBrojElementiVoOvojSplit<NeighCount) distances[j]=new
		 * Distance(j,tj,tempd); else { if (j<NeighCount) distances[j]=new
		 * Distance(j,tj,tempd); else{ biggestd = distances[0].distance; // go through
		 * the knn list and replace the biggest one if possible biggestindex = 0; for
		 * (int aa=1; aa<NeighCount; aa++) if (distances[aa].distance > biggestd)
		 ...
		 * (i==j) w[permutation[i]][permutation[j]]=0; else if
		 * ((distances[j].distance==0.0) && (i!=j)) w[permutation[i]][permutation[j]]=1;
		 * else{ switch (spatialMatrix) { case 0: w[permutation[i]][permutation[j]]=0;
		 * break; //binary case 1:
		 * w[permutation[i]][permutation[j]]=1/distances[j].distance; break; //euclidian
		 * default: w[permutation[i]][permutation[j]]=0; break; }
		 * W+=w[permutation[i]][permutation[j]]; } a[k] +=
		 * 2*w[permutation[i]][permutation[j]]; b[k] +=
		 * 2*w[permutation[i]][permutation[j]]*ti*tj; c[k] +=
		 * w[permutation[i]][permutation[j]]*tj; c[k] +=
		 * w[permutation[i]][permutation[j]]*tj; } m_PreviousSumW[k] =a[k];
		 * m_PreviousSumWXX[k]=b[k]; m_PreviousSumWX[k] =c[k]; m_PreviousSumX[k] =d[k];
		 * m_PreviousSumX2[k] =e[k]; } } if (a[k]!=0.0 &&
		 * e[k]!=(d[k]/(vkupenBrojElementiVoCelataSuma))){
		 * ikk[k]=(vkupenBrojElementiVoCelataSuma)*(b[k] -
		 * (2*d[k]*c[k]/(vkupenBrojElementiVoCelataSuma)) +
		 * (d[k]*d[k]*a[k]/((vkupenBrojElementiVoCelataSuma)*(
		 * vkupenBrojElementiVoCelataSuma))))/(a[k]*(e[k]-(d[k]/(
		 * vkupenBrojElementiVoCelataSuma)))); //I for each target avgik+=ikk[k];
		 * //average of the both targets }else avgik = 1; //Double.NEGATIVE_INFINITY; }
		 */
		avgik /= schema.getNbTargetAttributes();
		double I = avgik;
		if (I > 1)
	...
	public double calcEquvalentGDistance(Integer[] permutation) throws ClusException {
		...
		for (int k = 0; k < schema.getNbTargetAttributes(); k++) {
				...
				for (int i = M; i < NR; i++) {
					...
					for (int j = M; j < NR; j++) {
						...
						if (indexI != indexJ) {
							...
							m_PreviousSumWR[k] += 1;
							m_PreviousSumWXXR[k] += (xi - xj) * (xi - xj);
						}
					}
				}
			}
				...
				for (int i = 0; i < M; i++) {
					...
					for (int j = M; j < N; j++) {
						...
						if (indexI != indexJ) {
							...
							m_PreviousSumW[k] += 1;
							m_PreviousSumWXX[k] += (xi - xj) * (xi - xj);
						}
					}
				}
				// left (old-new)(0-old)
			...
			// System.out.println("Update SumLeft : sumX "+m_PreviousSumX[0]+" sumX2
			// "+m_PreviousSumX2[0]+" sumW
			// "+m_PreviousSumW[0]+" sumX2W "+m_PreviousSumWXX[0]+"
			// sumXW"+m_PreviousSumWX[0]);
			// System.out.println("Update SumRight : sumX "+m_PreviousSumXR[0]+" sumX2
			// "+m_PreviousSumX2R[0]+" sumW
			// "+m_PreviousSumWR[0]+" sumX2W "+m_PreviousSumWXXR[0]+"
			// sumXW"+m_PreviousSumWXR[0]);

			vkupenBrojElementiVoOvojSplit = N;
			num[k] = ((vkupenBrojElementiVoOvojSplit - 1) * m_PreviousSumWXX[k]);
			den[k] = (2 * m_PreviousSumW[k]
					* (m_PreviousSumX2[k]
							- vkupenBrojElementiVoOvojSplit * (m_PreviousSumX[k] / vkupenBrojElementiVoOvojSplit)
									* (m_PreviousSumX[k] / vkupenBrojElementiVoOvojSplit)));
			...
			denR[k] = (2 * m_PreviousSumWR[k] * (m_PreviousSumX2R[k]
					- (((m_PreviousSumXR[k] * m_PreviousSumXR[k])) / vkupenBrojElementiVoOvojSplit)));
			...
			// NM: "+(m_SplitIndex)+" W:
			// "+m_PreviousSumW[0]+" wx:"+m_PreviousSumWX[0]+" wxx:"+m_PreviousSumWXX[0]+"
			// xx:"+m_PreviousSumX2[0]);
			// System.out.println("Right Geary C: "+ikkR[0]+"numR "+numR[0]+"denR
			// "+denR[0]+" "+" NM:
			// "+(NR-m_SplitIndex)+" WR "+m_PreviousSumWR[0]+" wxR: "+m_PreviousSumWXR[0]+"
			// wxx "+m_PreviousSumWXXR[0]+"
			// xx:"+m_PreviousSumX2R[0]);
	...
	public double calcEquvalentIDistance(Integer[] permutation) throws ClusException {
		...
		for (int k = 0; k < schema.getNbTargetAttributes(); k++) {
				...
				for (int i = M; i < NR; i++) {
					...
					for (int j = M; j < NR; j++) {
						...
						if (indexI != indexJ) {
							...
							m_PreviousSumWR[k] += 1;
							m_PreviousSumWXXR[k] += xi * xj;
							m_PreviousSumWXR[k] += xj;
						}
					}
				}
				// System.out.println("Init SumLeft : sumX "+m_PreviousSumX[0]+" sumX2
				// "+m_PreviousSumX2[0]+" sumW
				// "+m_PreviousSumW[0]+" sumX2W "+m_PreviousSumWXX[0]+"
				// sumXW"+m_PreviousSumWX[0]);
				// System.out.println("Init SumRight : sumX "+m_PreviousSumXR[0]+" sumX2
				// "+m_PreviousSumX2R[0]+" sumW
				// "+m_PreviousSumWR[0]+" sumX2W "+m_PreviousSumWXXR[0]+"
				// sumXW"+m_PreviousSumWXR[0]);
			}
				...
				for (int i = 0; i < M; i++) {
					...
					for (int j = M; j < N; j++) {
						...
						if (indexI != indexJ) {
							...
							m_PreviousSumW[k] += 1;
							m_PreviousSumWXX[k] += xi * xj;
							m_PreviousSumWX[k] += xj;
						}
					}
				}
				// left (old-new)(0-old)
			...
			vkupenBrojElementiVoOvojSplit = N;
			num[k] = vkupenBrojElementiVoOvojSplit * (m_PreviousSumWXX[k]
					- 2 * (m_PreviousSumX[k] / vkupenBrojElementiVoOvojSplit) * m_PreviousSumWX[k]
					+ (m_PreviousSumX[k] / vkupenBrojElementiVoOvojSplit)
							* (m_PreviousSumX[k] / vkupenBrojElementiVoOvojSplit) * m_PreviousSumW[k]);
			den[k] = m_PreviousSumW[k] * (m_PreviousSumX2[k]
					- vkupenBrojElementiVoOvojSplit * (m_PreviousSumX[k] / vkupenBrojElementiVoOvojSplit)
							* (m_PreviousSumX[k] / vkupenBrojElementiVoOvojSplit));
			if (den[k] != 0 && num[k] != 0 && !flagLeftAllEqual)
				ikk[k] = num[k] / den[k]; // I for each target
			...
			vkupenBrojElementiVoOvojSplit = NR - N;
			numR[k] = (vkupenBrojElementiVoOvojSplit) * (m_PreviousSumWXXR[k]
					- (2 * m_PreviousSumWXR[k] * (m_PreviousSumXR[k] / vkupenBrojElementiVoOvojSplit))
					+ (((m_PreviousSumXR[k] / vkupenBrojElementiVoOvojSplit)
							* (m_PreviousSumXR[k] / vkupenBrojElementiVoOvojSplit)) * m_PreviousSumWR[k]));
			denR[k] = (m_PreviousSumWR[k] * (m_PreviousSumX2R[k]
					- (((m_PreviousSumXR[k] * m_PreviousSumXR[k])) / vkupenBrojElementiVoOvojSplit)));
			if (denR[k] != 0 && numR[k] != 0 && !flagRightAllEqual)
				ikkR[k] = numR[k] / denR[k]; // I for each target
			else
		...
		for (int k = 0; k < schema.getNbTargetAttributes(); k++) {
			...
			boolean flagLeftAllEqual = true;
			{
				...
				for (int i = 1; i < N; i++) {
					DataTuple exi = m_Data.getTuple(permutation[i]);
					double xi = type.getNumeric(exi);
					if (xi != oldX) // Annalisa : to check that partition does not contain values which are all the
									// same
					{
						flagLeftAllEqual = false;
						break;
	...
	// global I
	@Override
	public double calcItotal(Integer[] permutation) throws ClusException { // matejp: all tuple indices <index> are
																			// replaced by permutation[<index>]
		ClusSchema schema = m_Data.getSchema();
		double num, den;
		m_TempData = m_Data;
		int M = 0;
		...
		for (int k = 0; k < schema.getNbTargetAttributes(); k++) {
			...
			avgik += ikk[k]; // average of the both targets
			// System.out.println("w: "+W+"num: "+num+"den: "+den+"Left Moran I:
			// "+avgik+"ex: "+((N-M)));
		}
		avgik /= schema.getNbTargetAttributes();
		// System.out.println("Left Moran I: "+avgik+"ex: "+(N-M)+"W "+W+" means:
		...
		for (int k = 0; k < schema.getNbTargetAttributes(); k++) {
			...
			avgikR += ikkR[k]; // average of the both targets
			// System.out.println("w: "+WR+"num: "+num+"den: "+den+"Right Moran I:
			// "+avgikR+"ex: "+((N-M)));
		}
		avgikR /= schema.getNbTargetAttributes();
		// System.out.println("Right Moran I: "+avgikR+"ex: "+(N-M)+"W "+WR+" means:
		// "+meansR[0]);
		double IR = avgikR; // end right side
		double I = (IL + IR * (N - M)) / m_Data.getNbRows();
		double scaledI = 1 + I;
	...
	public double calcItotalD(Integer[] permutation) throws ClusException {
		...
		for (int k = 0; k < schema.getNbTargetAttributes(); k++) {
			...
			// System.out.println("w: "+W+"num "+num+"den "+den+"up: "+upsum[0]+"down:
			// "+downsum[0]+"Left Moran I:
			// "+avgik+"ex: "+((N-M)));
		}
		avgik /= schema.getNbTargetAttributes();
		double IL = avgik * (N - M);
		...
		for (int k = 0; k < schema.getNbTargetAttributes(); k++) {
			...
			if (num != 0.0 && den != 0.0) {
				...
				ikkR[k] = 1; // Double.NEGATIVE_INFINITY;
			avgikR += ikkR[k]; // average of the both targets
			// System.out.println("wR: "+WR+"num "+num+"den "+den+"upsum:
			// "+upsumR[0]+"downsum: "+downsumR[0]+"Right
			// Moran I: "+avgikR+"ex: "+((N-M)));
		}
		avgikR /= schema.getNbTargetAttributes();
		double IR = avgikR;
	...
	public double calcGtotal(Integer[] permutation) throws ClusException {
		...
		avgikR /= schema.getNbTargetAttributes();
		double IR = avgikR;
		// System.out.println("Right Geary C:"+IR+"examples: "+(N-M)+"WR: "+WR+"upR
		...
		for (int k = 0; k < schema.getNbTargetAttributes(); k++) {
			...
			for (int i = M; i < N; i++) {
				if (downsum[k] != 0.0 && upsum[k][permutation[i]] != 0.0) {
					...
					// "+i+","+upsum[k][permutation[i]]+"/"+(N-M)/downsum[k]);
					ikk[k] += ik[k][permutation[i]]; // average of all points for each targets
				} else
					ikk[k] = 0;
			}
		...
		for (int k = 0; k < schema.getNbTargetAttributes(); k++) {
			...
			for (int i = M; i < N; i++) {
				...
				if (upsum[k][permutation[i]] != 0.0) {
					ik[k][permutation[i]] = (upsum[k][permutation[i]]) / (downsum[k][permutation[i]]); // Local Getis
																										// for each
																										// point
					ikk[k] += ik[k][permutation[i]]; // average of all points for each targets
				} else
	...
	public double calcGETIStotal(Integer[] permutation) throws ClusException {
		...
		for (int k = 0; k < schema.getNbTargetAttributes(); k++) {
			...
			for (int i = M; i < N; i++) {
				...
				downsum2[k][permutation[i]] = (N - M) * downsum1[k][permutation[i]]
						- W[permutation[i]] * W[permutation[i]];
			...
			for (int i = M; i < N; i++) {
				if (downsum[k] != 0.0 && upsum[k][permutation[i]] != 0.0 && downsum2[k][permutation[i]] != 0.0) {
					...
					ik[k][permutation[i]] = upsum[k][permutation[i]]
							/ (Math.sqrt((downsum[k] / ((N - M) * (N - M - 1))) * downsum2[k][permutation[i]])); // I
																													// for
																													// each
																													// point
					ikk[k] += ik[k][permutation[i]]; // average of all points for each targets
