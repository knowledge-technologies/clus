...
import si.ijs.kt.clus.util.jeans.util.ThreadLock;
...
public abstract class AnimatedDnDCanvas extends DragNDropCanvas {
    ...
    public synchronized void startStopAnimator(boolean start) {
        if (m_animate_enabled == start)
            return;
        if (!m_paused && m_animate_mode == ANIMATE_MODE_THREAD)
            setAnimatorState(start);
    ...
    public synchronized void update(Graphics g) {
        ...
        if (isAnimated()) {
            ...
            if (m_draw_hint == HINT_ALL)
                paintBackground(bufGrp, bufSiz);
        }
        toScreen(g, bufSiz);
