...
import clus.data.type.primitive.IndexAttrType;
...
public class ClusSchema implements Serializable {
     ...
     * 
     * @param attruse
     *        The use type of attributes as defined in ClusAttrType. For example ATTR_USE_TARGET.
    ...
    public final boolean hasAttributeType(int attruse, int attrtype) {
        ...
        for (int i = 0; i < all.length; i++) {
            if (all[i].getTypeIndex() == attrtype)
                return true;
        }
        return false;
    }
    ...
    public final void checkRange(IntervalCollection coll, String type) throws ClusException {
        ...
        if (coll.getMaxIndex() > getNbAttributes())
            throw new ClusException("Range for " + type + " attributes: '" + coll + "' out of range (there are only " + getNbAttributes() + " attributes)");
    }
    ...
    public void initializeFrom(ClusSchema schema) throws ClusException {
        ...
        if (other_nb > this_nb) { throw new ClusException("To few attributes in data set " + this_nb + " < " + other_nb); }
        for (int i = 0; i < other_nb; i++) {
            ClusAttrType this_type = getAttrType(i);
            ClusAttrType other_type = schema.getAttrType(i);
            if (!this_type.getName().equals(other_type.getName())) { throw new ClusException("Attribute names do not align: '" + other_type.getName() + "' expected at position " + (i + 1) + " but found '" + this_type.getName() + "'"); }
            if (this_type.getClass() != other_type.getClass()) { throw new ClusException("Attribute types do not match for '" + other_type.getName() + "' expected '" + other_type.getClass().getName() + "' but found '" + this_type.getClass().getName() + "'"); }
        ...
        if (!m_IsSparse) {
            ...
            for (int i = 0; i < getNbAttributes(); i++) {
                ...
                if (type.isDescriptive() && type.getTypeIndex() == NumericAttrType.THIS_TYPE) {
                    ...
                    // nt.setClustering(true); //******************** //CV: added because of sparse Pert(?), removed
                    // because method ClusStatManager.check() gave error (2 types set in hierarchical context)
                    setAttrType(nt, i);
                    nbSparse++;
                }
                else {
    ...
    protected void addRowsIndex() {
        ...
        if (nbBitwise > 0)
            m_NbVt[ClusAttrType.VALUE_TYPE_BITWISEINT]++;
        m_NbInts = Math.max(m_NbVt[ClusAttrType.VALUE_TYPE_INT], m_NbVt[ClusAttrType.VALUE_TYPE_BITWISEINT]); // only
                                                                                                              // one of
                                                                                                              // them
                                                                                                              // will be
                                                                                                              // different
