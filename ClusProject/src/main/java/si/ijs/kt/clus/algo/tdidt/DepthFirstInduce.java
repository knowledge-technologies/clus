...
public class DepthFirstInduce extends ClusInductionAlgorithm {
	...
	public ClusAttrType[] getDescriptiveAttributes(ClusRandomNonstatic rnd) {
		...
		if (!sett.getEnsemble().isEnsembleMode()) {
			...
			case RForest:
				...
				selected = ClusEnsembleInduce.selectRandomSubspaces(attrsAll,
			...
			case ExtraTrees:// same as for Random Forests
				...
				selected = ClusEnsembleInduce.selectRandomSubspaces(attrs_all,
						schema.getSettings().getEnsemble().getNbRandomAttrSelected(),
						ClusRandomNonstatic.RANDOM_SELECTION, rnd);
	...
	public void induce(ClusNode node, RowData data, ClusRandomNonstatic rnd) throws Exception {
		...
		if (getSettings().getTree().getMissingClusteringAttrHandling()
				.equals(MissingClusteringAttributeHandlingType.EstimateFromParentNode)) {
		...
		for (int i = 0; i < attrs.length; i++) {
			...
			if ((getSettings().getEnsemble().isEnsembleMode())
					&& (getSettings().getEnsemble().getEnsembleMethod() == EnsembleMethod.ExtraTrees)) {
		...
		if (best.hasBestTest()) {
			...
			for (int j = 0; j < arity; j++) {
				...
				if (getSettings().getTree().getMissingClusteringAttrHandling()
						.equals(MissingClusteringAttributeHandlingType.EstimateFromParentNode)
						|| getSettings().getTree().getMissingTargetAttrHandling().equals(MissingTargetAttributeHandlingType.ParentNode)) {
	...
	public ClusNode induceSingleUnpruned(RowData data, ClusRandomNonstatic rnd) throws Exception {
		...
		while (true) {
			 ...
			 * file), otherwise this file is written for every tree, which is not conveniet
	...
	public ClusModel induceSingleUnpruned(ClusRun cr) throws Exception {
		...
		if (threads != 1) {
			String warning = String.format("Potential WARNING:\n" + "It seems that you are trying"
					...
					+ "The method induceSingleUnpruned(RowData data, ClusRandomNonstatic rnd) should be used instead.",
					threads);
