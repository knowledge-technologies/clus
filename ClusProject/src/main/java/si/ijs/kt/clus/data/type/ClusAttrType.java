...
public abstract class ClusAttrType implements Serializable, Comparable<ClusAttrType> {
    ...
    public enum AttributeType {
        Nominal("Nominal"), Numeric("Numeric"), Classes("Classes"), Index("Index"), String("String"), Integer("Integer"), TimeSeries("TimeSeries"), Set("Set"), Tuple("Tuple");
    ...
    public static ClusDistance createDistance(ClusAttrType type, Settings settings) {
        if (type instanceof TupleAttrType) {
            ...
            switch (settings.getTree().getTupleDistance()) {
                case Euclidean:
                    tupleDist = new si.ijs.kt.clus.distance.complex.tuples.EuclideanDistance((TupleAttrType) type, innerDistances);
                    break;
                case Minkowski:
            ...
            switch (settings.getTree().getTSDistance()) {
                case DTW:
                    tsDistance = new si.ijs.kt.clus.distance.primitive.timeseries.DTWTimeSeriesDist((TimeSeriesAttrType) type);
                    break;

                case QDM:
                    tsDistance = new si.ijs.kt.clus.distance.primitive.timeseries.QDMTimeSeriesDist((TimeSeriesAttrType) type);
                    break;

                case TSC:
                    tsDistance = new si.ijs.kt.clus.distance.primitive.timeseries.TSCTimeSeriesDist((TimeSeriesAttrType) type);
                    break;
            }
            return tsDistance;
        }
