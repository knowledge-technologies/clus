...
public class ColTarget {
    ...
    public void resize(int nbrows) {
        m_NbRows = nbrows;
        if (m_NbNumeric != 0)
            m_Numeric = new double[m_NbRows][m_NbNumeric];
        if (m_NbNominal != 0)
            m_Nominal = new int[m_NbRows][m_NbNominal];
    }
