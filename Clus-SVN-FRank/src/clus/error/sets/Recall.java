...
public class Recall extends ClusSetError {
    ...
    public Recall(ClusErrorList par, SetAttrType[] set, ClusDistance innerDistance) {
        this(par, set, null, true, innerDistance);
    }


    public Recall(ClusErrorList par, SetAttrType[] set, ClusAttributeWeights weights, ClusDistance innerDistance) {
        this(par, set, weights, true, innerDistance);
    }


    public Recall(ClusErrorList par, SetAttrType[] set, ClusAttributeWeights weights, boolean printall, ClusDistance innerDistance) {
        super(par, set);
    ...
    private void addExample(Set realSet, Set predicted) {
        double error = (SetOperations.getIntersectionCardinality(realSet, predicted, innerDistance) + 0.0) / predicted.size();
        if (realSet.size() == 0) {
            if (predicted.size() == 0) {
                error = 1;
            }
    ...
    public void showSummaryError(PrintWriter out, boolean detail) {
        NumberFormat fr = getFormat();
        out.println(getPrefix() + "Mean over components Recall: " + fr.format(getModelError()));
    }
    ...
    public String getName() {
        if (m_Weights == null)
            ...
            return "Weighted Recall (Recall) (" + m_Weights.getName(m_Attrs) + ")";
    }


    public ClusError getErrorClone(ClusErrorList par) {
        return new Recall(par, m_Attrs, m_Weights, m_PrintAllComps, innerDistance);
    }
