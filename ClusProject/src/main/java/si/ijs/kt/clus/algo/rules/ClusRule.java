...
public class ClusRule implements ClusModel, Serializable {
  ...
  public boolean equalsDeeply(Object other) {
    ...
    for (int iTarget = 0; iTarget < ruleTarget.length && targetsAreSimilar; iTarget++) {
      if (Math.abs(ruleTarget[iTarget] - ((RegressionStat) m_TargetStat).m_Means[iTarget])
          >= EQUAL_MAX_DIFFER) targetsAreSimilar = false;
    }
    ...
    if (m_TargetStat instanceof ClassificationStat) {
      ...
      for (int i = 0; i < nbRows; i++) {
        ...
        if (covers(tuple)) {
          ...
          int true_value;
          NominalAttrType[] targetAttrs =
              data.getSchema().getNominalAttrUse(ClusAttrType.ATTR_USE_TARGET);
      ...
      if (cov_method == SettingsRules.COVERING_METHOD_WEIGHTED_ADDITIVE) {
        ...
        if (cov_w_par == 1) {
          throw new ClusException(
              "Multiplicative weighted covering: covering weight should not be 1!");
        }
        ...
        if (m_TargetStat instanceof ClassificationStat) {
          ...
          NumericAttrType[] targetAttrs =
              data.getSchema().getNumericAttrUse(ClusAttrType.ATTR_USE_TARGET);
          if (predictions.length > 1) { // Multiple target
            ...
            for (int j = 0; j < true_values.length; j++) {
              ...
              coef[j] =
                  cov_w_par
                      * Math.abs(predictions[j] - true_values[j])
                      / Math.sqrt(variance[j]); // Add
              // /2
              // or
              // /4
              // here?
            ...
            double coef =
                cov_w_par * Math.abs(prediction - true_value) / Math.sqrt(variance); // Add /2 or
            // /4 here?
            if (coef > 1) { // Limit max weight to 1
              coef = 1;
            }
  ...
  public int getNumberOfViolatedConstraints() {
    ...
    while (i.hasNext()) {
      ...
      if (ilc.getType() == 0) {
        // ML
        if (!(m_Data.contains(t1) && m_Data.contains(t2))) count++;
      } else {
        if (m_Data.contains(t1) && m_Data.contains(t2)) count++;
      }
    }
  ...
  public int getNumberOfViolatedConstraintsRCCC() {
    ...
    while (i.hasNext()) {
      ...
      if (ilc.getType() == 0) {
        // ML
        if (!(data.contains(t1) && data.contains(t2))) count++;
      } else {
        // CL
        if (data.contains(t1) && data.contains(t2)) count++;
      }
  ...
  public ArrayList<ILevelConstraint> getViolatedConstraintsRCCC() {
    ...
    while (i.hasNext()) {
      ...
      if (ilc.getType() == 0) {
        // ML
        if (!(data.contains(t1) && data.contains(t2))) c.add(ilc);
      } else {
        // CL
        if (data.contains(t1) && data.contains(t2)) c.add(ilc);
      }
  ...
  protected void commonPrintForRuleTypes(PrintWriter wrt, StatisticPrintInfo info) {
    ...
    if (getSettings().getRules().computeDispersion() && (m_CombStat[ClusModel.TRAIN] != null)) {
      ...
      wrt.println(
          "   Cover*Disp (train): "
              + fr.format(
                  (m_CombStat[ClusModel.TRAIN].dispersionCalc() * m_Coverage[ClusModel.TRAIN])));
      if (m_CombStat[ClusModel.TEST] != null) {
        ...
        wrt.println(
            "   Cover*Disp (test):  "
                + fr.format(
                    (m_CombStat[ClusModel.TEST].dispersionCalc() * m_Coverage[ClusModel.TEST])));
      }
    }
    if (hasErrors()) {
  ...
  @Override
  public void printModelToQuery(
      PrintWriter wrt, ClusRun cr, int starttree, int startitem, boolean ex) {}
  ...
  public void setNumericPrediction(double[] newPred) {
    ...
    for (int iTarget = 0; iTarget < stat.getNbAttributes(); iTarget++) {
      ...
      stat.setSumValues(
          iTarget,
          stat.m_Means[iTarget]); // stat.m_SumValues[iTarget] = stat.m_Means[iTarget];
  ...
  public void setTrainErrorScore() throws ClusException {
    ...
    if (m_TargetStat instanceof ClassificationStat) {
      ...
      NominalAttrType[] targetAttrs =
          m_StatManager.getSchema().getNominalAttrUse(ClusAttrType.ATTR_USE_TARGET);
      ...
      double norm =
          getSettings()
              .getRules()
              .getVarBasedDispNormWeight(); // For RRMSE this * std dev is the normalization
      ClusStatistic stat = m_StatManager.getTrainSetStat();
      NumericAttrType[] targetAttrs =
          m_StatManager.getSchema().getNumericAttrUse(ClusAttrType.ATTR_USE_TARGET);
      ...
      for (int i = 0; i < nb_rows; i++) {
        ...
        for (int j = 0; j < nb_tar; j++) {
          diff[j] +=
              Math.abs(prediction[j] - targetAttrs[j].getNumeric(tuple)); // |prediction-true|
        }
      }
      double sum_diff = 0.0;
      for (int j = 0; j < nb_tar; j++) {
  ...
  public boolean coversCorrect(DataTuple tuple) {
    ...
    if (m_CoverageBits.get(tuple.getIndex())) {
      return m_CoverageCorrectBits.get(tuple.getIndex());
    }
