...
public class GISHeuristic extends ClusHeuristic {
    ...
    public GISHeuristic(ClusAttributeWeights prod, ClusAttrType[] attrs) {
            ...
            double[][] w = new double[N][N]; 
                ...
                for (int i = 0; i < N; i++) {
                    ...
                    for (int j = 0; j < N; j++) {
                        ...
                        Boolean longlat = schema.getSettings().isLonglat(); 
                        if (longlat==true) d=LanLongdistance(xi,yi,xj,yj);
                            else d = Math.sqrt((xi-xj)*(xi-xj)+(yi-yj)*(yi-yj));    
                ...
                double bandwidth = schema.getSettings().getBandwidth(); 
                ...
                for (int i = 0; i < N; i++) {
                    for (int j = 0; j < N; j++) {
                        ...
                        Boolean longlat = schema.getSettings().isLonglat(); 
                        if (longlat==true) d=LanLongdistance(xii,yii,xji,yji);
                            else d = Math.sqrt((xii-xji)*(xii-xji)+(yii-yji)*(yii-yji));    
    ...
    public void readMatrixFromFile(RowData data) throws IOException {
        ...
        double bandwidth = schema.getSettings().getBandwidth(); double w;
        if (bandwidth!=100.0){            
            while (br.ready()){
            ...
            while (br.ready()){
                ...
                int spatialMatrix = schema.getSettings().getSpatialMatrix();    
                switch (spatialMatrix) {
                    case 0:  if (d==1) w=1; else w=0;  break;  //binary 
                    case 1:  if (d==1) w=0.01; else w=1-1/d; break;  //euclidian
                    default: w=0; break;
                 }                                        
    ...
    public double calcI(ClusStatistic t_stat, ClusStatistic p_stat, ClusStatistic missing, Integer[] permutation) {
        ...
        ClusStatistic pstat = (ClusStatistic)p_stat;
            // Acceptable?
          if (stopCriterion(t_stat, p_stat, missing)) {
            ...
            case 2:  ss_pos = pstat.calcGetisTotal(permutation); break;//"Global Getis"
            case 3:  ss_pos = pstat.calcLISAtotal(permutation); break; //"Local Moran"
            case 4:  ss_pos = pstat.calcGLocalTotal(permutation);  break; //"Local Geary"
            case 5:  ss_pos = pstat.calcLocalGetisTotal(permutation); break;//"Local Getis"
            case 6:  ss_pos = pstat.calcGETIStotal(permutation); break;//Local Standardized Getis
            case 7:  ss_pos = pstat.calcEquvalentItotal(permutation);break;//EquvalentI 
            case 8:  ss_pos = pstat.calcIwithNeighbourstotal(permutation); break; //Global Moran with neigh
            case 9:  ss_pos = pstat.calcEquvalentIwithNeighbourstotal(permutation); break; // EquvalentI Global Moran with neigh
            case 10:  ss_pos = pstat.calcItotalD(permutation); break; //"Global Moran with a separate distance file"
            case 11:  ss_pos = pstat.calcGtotalD(permutation);  break; //"Global Geary with a separate distance file"
            case 12:  ss_pos = pstat.calcCItotal(permutation); break; // Connectivity Index (CI) for Graph Data
            case 13:  ss_pos = pstat.calcMutivariateItotal(permutation); break; //Cross Moran (Wartenberg, 1985)
            case 14:  ss_pos = pstat.calcCwithNeighbourstotal(permutation); break; //Global Geary with neigh
            case 15:  ss_pos = pstat.calcBivariateLee(permutation); break; //Bivariate Lee's measure: integration of Moran&Pearson coef. (Lee, 2001)
            case 16:  ss_pos = pstat.calcMultiIwithNeighbours(permutation); break; //Cross Moran (Wartenberg, 1985) with neigh
            case 17:  ss_pos = pstat.calcCIwithNeighbours(permutation); break; // Connectivity Index (CI) for Graph Data with neigh
            case 18:  ss_pos = pstat.calcLeewithNeighbours(permutation); break; //Bivariate Lee's measure with neigh
            case 19:  ss_pos = pstat.calcPtotal(permutation); break; //Global Moran without weights i.e Pearson c. coef.
            case 20:  ss_pos = pstat.calcCItotalD(permutation); break; // Connectivity Index (CI) for Graph Data with a separate distance file
            case 21:  ss_pos = pstat.calcDHtotalD(permutation); break; // Dyadicity and Heterophilicity for Graph Data with a separate distance file
            case 22:  ss_pos = pstat.calcEquvalentIDistance(permutation); break; // EquvalentI with a separate distance file
            case 23:  ss_pos = pstat.calcPDistance(permutation); break; //Pearson c. coef. with a separate distance file
            case 24:  ss_pos = pstat.calcEquvalentGtotal(permutation); break; //EquvalentG, (Global Geary)
            case 25:  ss_pos = pstat.calcEquvalentGDistance(permutation); break; //EquvalentGDistance, EquvalentG with a separate distance file
            case 26:  ss_pos = pstat.calcEquvalentPDistance(permutation); break; //EquvalentGDistance, EquvalentP with a separate distance file
            
            default: SpatialMeasure=0; break;
        }
            return ss_pos;
                    /*}
        ...
        public double calcHeuristic(ClusStatistic c_tstat, ClusStatistic[] c_pstat, int nbsplit) {            
            ...
            if (c_tstat instanceof ClassificationStat){
                ...
                double n_tot = tstat.getTotalWeight(); value = tstat.entropy();
                ...
                si /= MathUtil.M_LN2;
                if (si < MathUtil.C1E_6) return Double.NEGATIVE_INFINITY; // Return calculated gainratio
                value=value/si;
                }
            }
            else{
                if(c_tstat instanceof RegressionStat){
                    ...
                    RegressionStat tstat = (RegressionStat)c_tstat;
                    RegressionStat[] pstat = (RegressionStat[])c_pstat;    
                    for (int i = 0; i < nbsplit; i++) ss_sum += pstat[i].getSVarS(m_ClusteringWeights);
                    double ss_tot = tstat.getSVarS(m_ClusteringWeights);
