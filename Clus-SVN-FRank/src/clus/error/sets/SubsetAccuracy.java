...
import clus.ext.structuredTypes.SetOperations;
...
public class SubsetAccuracy extends ClusSetError {
    ...
    public SubsetAccuracy(ClusErrorList par, SetAttrType[] set, ClusDistance innerDistance) {
        this(par, set, null, true, innerDistance);
    }


    public SubsetAccuracy(ClusErrorList par, SetAttrType[] set, ClusAttributeWeights weights, ClusDistance innerDistance) {
        this(par, set, weights, true, innerDistance);
    }


    public SubsetAccuracy(ClusErrorList par, SetAttrType[] set, ClusAttributeWeights weights, boolean printall, ClusDistance innerDistance) {
        super(par, set);
    ...
    private void addExample(Set realSet, Set predicted) {
        double error = 0;
        if (realSet.size() != predicted.size()) {
            error = 0;
        }
        else {
            error = SetOperations.getIntersectionCardinality(realSet, predicted, innerDistance) == predicted.size() ? 1 : 0;
        }
        m_SumErr[0] += error;
        m_nbEx[0]++;
    }
    ...
    public void add(ClusError other) {
        SubsetAccuracy oe = (SubsetAccuracy) other;
        for (int i = 0; i < m_Dim; i++) {
    ...
    public void showSummaryError(PrintWriter out, boolean detail) {
        NumberFormat fr = getFormat();
        out.println(getPrefix() + "Mean over components Subset ACC: " + fr.format(getModelError()));
    }


    public String getName() {
        if (m_Weights == null)
            return "Subset Accuracy (SACC)";
        else
    ...
    public ClusError getErrorClone(ClusErrorList par) {
        return new SubsetAccuracy(par, m_Attrs, m_Weights, m_PrintAllComps, innerDistance);
    }
