    ...
    private class MyDrawableCnv extends DrawableCanvas {
        ...
        public void autoScrollTo() {
            if (m_ScrollTo != null) {
                ...
                if (left < getXOrig())
                    deltax = Math.min(deltax, left - getXOrig());
                if (right > scr_right)
                    deltax = Math.max(deltax, right - scr_right);
                if (top < getYOrig())
                    deltay = Math.min(deltay, top - getYOrig());
                if (bottom > scr_bottom)
                    deltay = Math.max(deltay, bottom - scr_bottom);
                if (deltax != 0) {
