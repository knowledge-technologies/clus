...
/**
 * Class representing a optimization problem. Can be used e.g. in Differential
 * evolution and Gradient descent optimization Includes loss functions and
 * regularization
 * 
 * @author Timo Aho
 * @author Tea Tusar
...
public class OptimizationProblem {
	...
	/**
	 * A regression prediction for initialization. It is negative so that this class
	 * is not selected in classification. Because default rule is used, this value
	 * should never go to loss function.
	 */
	static protected final double INVALID_PREDICTION = Double.NEGATIVE_INFINITY;

	/**
	 * Parameters for optimization algorithm. Predictions for rules and other base
	 * function and true values for all the instances of data. The default
	 * prediction is used, if no other rule covers the instance. See m_RulePred and
	 * m_TrueVal for what the indices mean.
	 */
	static public class OptimizationParameter {

		/** An empty class */
		public OptimizationParameter(int nbRule, int nbOtherBaseFunc, int nbInst, int nbTarg,
				ImplicitLinearTerms implicitLinTerms) {
		...
		public OptimizationParameter(RulePred[] rulePredictions, double[][][][] predictions, TrueValues[] trueValues,
				ImplicitLinearTerms implicitLinTerms) {
		...
		// public OptParam(RulePred[] rulePredictions, double[][][][] predictions,
		// double[][] trueValues
		// ){m_rulePredictions = rulePredictions; m_baseFuncPredictions = predictions;
		// m_trueValues = trueValues;}
	...
	/**
	 * Predictions of rule type base functions. Includes the prediction and boolean
	 * array of covered instances.
	 */
	static public class RulePred {

		/** An empty class */
		// public RulePred(int nbInst, int nbTarg){m_cover = new
		// boolean[nbInst];m_prediction = new double[nbTarg][1];}
		// public RulePred(boolean[] cover, double[][] prediction){m_cover =
		// cover;m_prediction=prediction;}

		/*
		 * public RulePred(int nbInst, int nbTarg){m_cover = new
		 * WAHBitSet();m_prediction = new double[nbTarg][1];} public RulePred(WAHBitSet
		 * cover, double[][] prediction){m_cover = cover;m_prediction=prediction;}
		 * //public boolean[] m_cover; // [instance] that are covered public WAHBitSet
		 * m_cover; // [instance] that are covered public double[][] m_prediction; //
		 * [target index][class value]
		 */
		...
		/*
		 * public RulePred(BitSet cover, double[][] prediction) { m_cover = cover;
	...
	/**
	 * Rule indexes are always first. Rule predictions [rule index]. Includes the
	 * prediction and boolean array of covered instances. Similar to m_BaseFuncPred
	 * but only for rules (constant prediction if covers).
	 */
	private RulePred[] m_RulePred;

	/**
	 * Other base function predictions [function index][instance][target
	 * index][class_value] is for nominal attributes or [function
	 * index][instance][target index][0] for regression. The [target index] is
	 * always [0] for single target use. The class value dimension is dynamic for
	 * each target index. These base functions have always indices AFTER rules. For
	 * rules use the other array, it is faster!
	 */
	private double[][][][] m_BaseFuncPred;

	/**
	 * If we want to save memory (amount of linear terms may be huge) we are not yet
	 * explicitly adding the linear terms to the set. Instead we are asking from the
	 * linear term set for the prediction. This is used only if a switch is set on.
	 */
	private ImplicitLinearTerms m_LinTermMemSavePred = null;

	/**
	 * True values of the instances. Includes targets for single line (example) and
	 * maybe a reference (if implicit linear terms are used) for the line this
	 * represents.
	 */
	...
	/**
	 * Do we use save memory version of linear terms. A very often used function
	 * call
	 */
	boolean m_saveMemoryLinears;

	/**
	 * Constructor for problem to be solved with optimization. Both classification
	 * and regression.
	 * 
	...
	public OptimizationProblem(ClusStatManager stat_mgr, OptimizationParameter optInfo) {
		...
		m_saveMemoryLinears = getSettings().getRules().getOptAddLinearTerms()
				.equals(OptimizationGDAddLinearTerms.YesSaveMemory);
		...
		if (stat_mgr.getTargetMode() != ClusStatManager.Mode.REGRESSION
			...
			// for classifying.\n" +
			// "Mixed targets are not yet implemented. The targets are considered as
			// regression.\n" +
		...
		if (!m_ClssTask) { // regression
			...
			if (getSettings().getRules().isOptNormalization()) {
				...
				// m_TargetNormFactor = computeOptNormFactors(means, varIsNonZero,
				// valuesFor0Variance);
	...
	public static void splitDataIntoValAndTrainSet(ClusStatManager stat_mgr, OptimizationParameter origData,
		...
		// We are using Fitness function of the problem. Let us put the reg penalty to 0
		// because we do not
		// want to use it
	...
	/**
	 * Fitness function over all targets. The classification prediction is voting
	 * without weights. Smaller is better.
	 * 
	 * @param genes The current generation (population).
	 * @return fitness score
	 */

	final public double calcFitness(ArrayList<Double> genes, ClusRuleSet rset) {
		return calcFitnessForTarget(genes, -1, rset);
	}

	/**
	 * Fitness function for single target, not for all of them. The classification
	...
	public double calcFitnessForTarget(ArrayList<Double> genes, int iFitnessTarget, ClusRuleSet rset) {
		...
		// TODO mixture of nominal and numeric attributes. For multi target case some of
		// the targets may be nominal,
		// some numerical
		...
		/**
		 * Number of values for each target. For regression classes are not needed, thus
		 * value is 1.
		 */
		...
		if (isClassifTask()) {
			// Only one sensible loss type available for classification and it is not the
			// default.
...
//		// average rule width (number of conjuncts)
//		double avgWidth = 0d;
//		int ruleCount = 0;
//		ClusRule rule;
//		for (int iRule = 1; iRule < getNumVar(); iRule++) {
//			if (genes.get(iRule).doubleValue() != 0d) {
//				ruleCount++;
//				rule = rset.getRule(iRule);
//				
//				avgWidth += rule.getModelSize();
//			}
//		}
//		if (avgWidth != 0d) {
//			avgWidth /= ruleCount;
//		}
	...
	private double[] predictRegression(double[][] predictionSums) {
		...
		for (int iTarget = 0; iTarget < nbOfTargets; iTarget++) {
			if (predictionSums[iTarget] != null) {
				// If internal normalization has been used, we need to use the offset parameter
				// to
				// shift the prediction to right place. If internal normalization is not used,
	...
	protected double[] predictClass(double[][] predictionSums) {
		...
		for (int iTarget = 0; iTarget < nbOfTargets; iTarget++) {
			...
			// // Thus no prediction is given (none of the nonzero weight rules cover the
			// instance)
			// // Use the default rule instead
			// prediction[iTarget] = getDefaultPrediction(iTarget);
			// } else {

			// The default rule is included in the rule set. At least it always covers all
			// the examples.
	...
	/**
	 * Relative root mean squared error RRMSE loss function for data set. RRMSE is
	 * sum of squared errors divided by the variance. Relativeness is not useful,
	...
	/**
	 * Huber 1962 loss function for data set. This is mainly the squared distance
	 * error. However for great distances it is smoothed. Thus it is robust to
	 * outliers.
	 * 
	 * @param trueValue  The true values of targets. [Instance][Target]
	 * @param prediction The prediction. [Instance][Target]
	...
	private double lossHuber(TrueValues[] trueValue, double[][] prediction) {
		...
		for (int jTarget = 0; jTarget < numberOfTargets; jTarget++) {
			...
			for (int iInstance = 0; iInstance < numberOfInstances; iInstance++) {
				if (Math.abs(
						trueValue[iInstance].m_targets[jTarget] - prediction[iInstance][jTarget]) < deltas[jTarget]) {
					attributeLoss += Math.pow(trueValue[iInstance].m_targets[jTarget] - prediction[iInstance][jTarget],
							2);
				} else { // Smoothed for distant objects
					attributeLoss += deltas[jTarget]
							* (Math.abs(trueValue[iInstance].m_targets[jTarget] - prediction[iInstance][jTarget])
									- deltas[jTarget] / 2);
				}
			}
	...
	private double[] computeHuberDeltas(TrueValues[] trueValues, double[][] predictions) {
		...
		for (int jTarget = 0; jTarget < numberOfTargets; jTarget++) {
			for (int iInstance = 0; iInstance < numberOfInstances; iInstance++) {
				targetDistances[iInstance] = Math
						.abs(trueValues[iInstance].m_targets[jTarget] - predictions[iInstance][jTarget]);
			}
	...
	/**
	 * 0-1 distance loss function for data set. Usually used only in classification.
	 * 
	 * @param trueValue  The true values of targets. [Instance][Target]
	 * @param prediction The prediction. [Instance][Target]
	 * @return Loss for the data.
	...
	/**
	 * Regularization penalty for the optimization function. This reduces the size
	 * of genes so that they do not grow too much. The power for the differences can
	 * be changed in settings file. The default is lasso, power = 1. This keeps the
	 * weights zero if possible.
	 * 
	...
	protected double regularization(ArrayList<Double> genes) {
		...
		for (int j = 0; j < genes.size(); j++) {
			...
			reg_penalty += Math.pow(Math.abs((genes.get(j)).doubleValue()),
					getSettings().getRules().getOptDERegulPower());
		}
	...
	/**
	 * Does the base function cover the given example? This should be used instead
	 * of checkin if prediction is NaN.
	 */
	final protected boolean isCovered(int iRule, int iInstance) {
		if (iRule >= m_RulePred.length) {
			return m_saveMemoryLinears ? true
					: !Double.isNaN(m_BaseFuncPred[iRule - m_RulePred.length][iInstance][0][0]);
			// if (m_saveMemoryLinears) {
	 ...
	 * when we already know that instance is covered! Use isCovered for this. If
	 * used right, always gives prediction.
	 */
	final protected double getPredictionsWhenCovered(int iRule, int iInstance, int iTarget, int iClass) {
		if (!isRuleTerm(iRule)) {
			if (m_saveMemoryLinears) {
				/* Nb of targets */
				return m_LinTermMemSavePred.predict(iRule - m_RulePred.length, m_TrueVal[iInstance].m_dataExample,
						iTarget, m_RulePred[0].m_prediction.length);
			} else {
	...
	/**
	 * Value of base function prediction in regression use when we already know that
	 * instance is covered! Use isCovered for this. If used right, gives always
	 * prediction.
	 */
	...
	/**
	 * The loss function should be separate in the sense that it does not use member
	 * functions. OBSOLETE, does not take into account target avg even if it should!
	 * 
	 * 
	 */
	...
	/**
	 * Change the data used for learning. This is used if part of the data is used
	 * for e.g. testing.
	 */
	...
	/**
	 * Initializes norm factors based on precomputed std dev values. Norm factor is
	...
	/**
	 * For normalization during optimization, the scaling factor for each of the
	 * targets. For single prediction/true value Math.sqrt() of this should be used,
	 * because this is meant for e.g. covariance computing.
	 */
	...
	/**
	 * Prepare predictions for normalization, the predictions might have to be
	 * changed
	 */
	protected void preparePredictionsForNormalization() {

		// Preparations are needed only if some normalization with average shifting is
		// used.
		if (!getSettings().getRules().isOptNormalization()
		...
		for (int iTarg = 0; iTarg < getNbOfTargets(); iTarg++) {
			...
			if (getPredictionsWhenCovered(0, 0, iTarg) != getMean(iTarg)) {
				System.err.println("Error: Difference in preparePredictionsForNormalization for target nb " + iTarg
						+ ". The values are " + getPredictionsWhenCovered(0, 0, iTarg) + " and " + getMean(iTarg));
				System.exit(1);
			}
		...
		if (GDProblem.m_printGDDebugInformation) {
			...
			try (PrintWriter wrt_pred = new PrintWriter(
					new OutputStreamWriter(new FileOutputStream(fname + ".gd-pred")));
					PrintWriter wrt_true = new PrintWriter(
							new OutputStreamWriter(new FileOutputStream(fname + ".gd-true")))) {
