...
class Numero {
	double x;double y;double v;double v1;
	Numero(double x,double y,double v,double v1){this.x=x;this.y=y;this.v=v; this.v1=v1;}
}
...
public class PredictionAnalyzer {
	...
	public static float weight(int i, int j, float bandwidth){
		if(D[i][j]==0)
			return 0;
				...
				if(D[i][j]<=bandwidth)
					return 
					1-(((float)Math.sqrt((float)(Math.pow(punti[i].x-punti[j].x, 2)+Math.pow(punti[i].y-punti[j].y, 2))))/bandwidth);	
				else				
					return 0;
	...
	int n=0; N=fileSize; D= new float[N][N];punti=new Numero[N];puntiM=new Numero[N];
	while (br.ready()){
		String s = br.readLine();
		StringTokenizer st = new StringTokenizer(s,",");
