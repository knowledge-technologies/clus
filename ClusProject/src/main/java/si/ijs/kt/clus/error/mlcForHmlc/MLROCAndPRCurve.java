...
public abstract class MLROCAndPRCurve {
    ...
    public double getCurveError(CurveType typeOfCurve) {
        ...
        throw new RuntimeException("Unknown type of curve: " + typeOfCurve);
    }
    ...
    public void add(MlcHmlcSubError other) {
        ...
        for (int i = 0; i < m_ROCAndPRCurves.length; i++) {
            m_ClassWisePredictions[i].add(o.m_ClassWisePredictions[i]);
        }
        computeAll();
    }
}
