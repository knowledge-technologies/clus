...
import si.ijs.kt.clus.main.settings.section.SettingsTree.EntropyType;
...
public class ClassificationStat extends ClusStatistic implements ComponentStatistic {
	...
	private int m_NbAttrs; // daniela // matejp: Inspect the code and you will see that m_NbAttrs = 0 for
							// ever and ever
	...
	// daniela matejp: public --> private, static INIT_PART_SUM ---> no longer
	// static.
	public double m_I;
	 ...
	 */
	public ClassificationStat(Settings sett, NominalAttrType[] nomAtts,
			INIFileNominalOrDoubleOrVector multiLabelThreshold) {
		this(sett, nomAtts);
	...
	/**
	 * Returns the nominal attribute. Added because RegressionStat has this method.
	 * 
	 * @param idx
	 *            index of attribute.
	 ...
	 * <p>
	 * but in voting procedure (see, e.g. {@link #voteProbDistr(ArrayList)}, the
	 * counts are normalized whereas the sums of weights are not, hence, the label
	 * is considered relevant IFF
	 * <p>
	 * 
	...
	public double entropyDifference(ClassificationStat other, ClusAttributeWeights scale) {
		...
		for (int i = 0; i < m_NbTarget; i++) {
			...
			if (other.getAttribute(i).getSchema().getSettings().getTree().getEntropyType()
					.equals(EntropyType.StandardEntropy)) {
				sum += entropyDifference(i, other);
			} else {
	...
	public double entropy(ClusAttributeWeights scale) {
		...
		double sum = 0.0;
		for (int i = 0; i < m_NbTarget; i++) {
			if (getAttribute(i).getSchema().getSettings().getTree().getEntropyType()
					.equals(EntropyType.StandardEntropy)) {
				sum += entropy(i);
			} else {
	...
	/**
	 * If there is no examples for an attribute i, gini is estimated according to
	 * setting given in settings file
	 * 
	 * @param i
	...
	public double getProportion(int attr, int cls) {
		double total = m_SumWeights[attr];
		if (total <= MathUtil.C1E_9) {
			// no examples -> assume training set distribution | matejp: there is a bug here
			// ... Pray that the program does not visit this branch:)
	...
	public static double computeSplitInfo(double sum_tot, double sum_pos, double sum_neg) {
		...
		return -(sum_pos / sum_tot * Math.log(sum_pos / sum_tot) + sum_neg / sum_tot * Math.log(sum_neg / sum_tot))
				/ MathUtil.M_LN2;
	}
	 ...
	 *         samples that have known value (0 or 1) for label I.
	 */
	...
	/**
	 * Computes a G statistic and returns the p-value of a G test. G =
	 * 2*SUM(obs*ln(obs/exp))
	 * 
	 * @param att
	 *            attribute index
	 * @return p-value
	...
	public double getGTestPValue(int att, ClusStatManager stat_manager) {
		...
		// distributionFactory.createChiSquareDistribution(degreesOfFreedom);
	...
	/**
	 * Computes a G statistic and returns the result of a G test. G =
	 * 2*SUM(obs*ln(obs/exp))
	 * 
	 * @param att
	 *            attribute index
	...
	public String getPredictedClassName(int idx) {
		// return m_Attrs[idx].getName()+" =
		// "+m_Attrs[idx].getValue(m_MajorityClasses[idx]);
	...
	/*
	 * Compute squared distance between each of the tuple's target attributes and
	 * this statistic's mean.
	 **/
				...
				for (int i = M; i < N; i++) {
					...
					for (int j = M; j < NN; j++) {
						...
						upsum[k] += w[permutation[i]][permutation[j]] * (ti - means[k])
								* (distances[j].target - means[k]); // m_distances.get(permutation[i]*N+permutation[j])
				...
				for (int i = M; i < N; i++) {
					...
					while ((distances.length > j) && (distances[j] != null) && j < NeighCount) {
						...
						upsumR[k] += w[permutation[i]][permutation[j]] * (ti - meansR[k])
								* (distances[j].target - meansR[k]); // m_distances.get(permutation[i]*N+permutation[j])
																		// [permutation[i]][permutation[j]]
			...
			for (int j = M; j < N; j++) {
				...
				if (permutation[i] != permutation[j]) {
					...
					if (temp != null)
						...
						w = 0;
					// upsum[0][permutation[i]] += w*xj[0]; upsum[1][permutation[i]] += w*xj[1];
					// W+=w*w;
	...
	public double calcLeewithNeighbours(Integer[] permutation) throws ClusException {
		...
		if (NeighCount > 0) {
			...
			for (int i = M; i < N; i++) {
				...
				double W = 0;
				for (int k = 0; k < schema.getNbTargetAttributes(); k++) {
					ClusAttrType type = schema.getNominalAttrUse(AttributeUseType.Target)[k]; // target
					ClusAttrType xt = schema.getNumericAttrUse(AttributeUseType.GIS)[0]; // x coord
					ClusAttrType yt = schema.getNumericAttrUse(AttributeUseType.GIS)[1]; // y coord
					xi[k] = type.getNumeric(exi);
			...
			for (int i = M; i < N; i++) {
				for (int j = M; j < N; j++)
					if (permutation[i] != permutation[j] && DR[(permutation[j] - M)] != 0)
						WR[(permutation[i] - M)] += wr[(permutation[i] - M)][(permutation[j] - M)]
								/ Math.sqrt(DR[(permutation[j] - M)]);
	...
	public double calcEquvalentIwithNeighbourstotal(Integer[] permutation) throws ClusException {
		...
		/*
		 * for (int k = 0; k < schema.getNbTargetAttributes(); k++) {
		 * a[k]=previousSumW[k]; b[k]=previousSumWXX[k]; c[k]=previousSumWX[k];
		 * d[k]=previousSumX[k]; e[k]=previousSumX2[k]; int NeighCount =
		 ...
		 * = schema.getNominalAttrUse(AttributeUseType.Target)[k]; DataTuple exi =
		 ...
		 * else{ switch (spatialMatrix) { case 0: w[i][j]=0; break; //binary case 1:
		 * w[i][j]=1/distances[j].distance; break; //euclidian default: w[i][j]=0;
		 * break; } W+=w[i][j]; } a[k] += 2*w[i][j]; b[k] += 2*w[i][j]*ti*tj; c[k] +=
		 * w[i][j]*tj; c[k] += w[i][j]*tj; } previousSumW[k] =a[k];
		 * previousSumWXX[k]=b[k]; previousSumWX[k] =c[k]; previousSumX[k] =d[k];
		 * previousSumX2[k] =e[k]; } } if (a[k]!=0.0 &&
	...
	public double calcEquvalentGDistance(Integer[] permutation) throws ClusException {
		...
		for (int k = 0; k < schema.getNbTargetAttributes(); k++) {
			...
			den[k] = (2 * previousSumW[k]
					* (previousSumX2[k]
							- vkupenBrojElementiVoOvojSplit * (previousSumX[k] / vkupenBrojElementiVoOvojSplit)
									* (previousSumX[k] / vkupenBrojElementiVoOvojSplit)));
			...
			denR[k] = (2 * previousSumWR[k]
					* (previousSumX2R[k] - (((previousSumXR[k] * previousSumXR[k])) / vkupenBrojElementiVoOvojSplit)));
	...
	public double calcEquvalentIDistance(Integer[] permutation) throws ClusException {
		...
		for (int k = 0; k < schema.getNbTargetAttributes(); k++) {
			...
			vkupenBrojElementiVoOvojSplit = N;
			num[k] = vkupenBrojElementiVoOvojSplit
					* (previousSumWXX[k] - 2 * (previousSumX[k] / vkupenBrojElementiVoOvojSplit) * previousSumWX[k]
							+ (previousSumX[k] / vkupenBrojElementiVoOvojSplit)
									* (previousSumX[k] / vkupenBrojElementiVoOvojSplit) * previousSumW[k]);
			den[k] = previousSumW[k] * (previousSumX2[k]
					- vkupenBrojElementiVoOvojSplit * (previousSumX[k] / vkupenBrojElementiVoOvojSplit)
							* (previousSumX[k] / vkupenBrojElementiVoOvojSplit));
			if (den[k] != 0 && num[k] != 0 && !flagLeftAllEqual)
				ikk[k] = num[k] / den[k]; // I for each target
			...
			vkupenBrojElementiVoOvojSplit = NR - N;
			numR[k] = (vkupenBrojElementiVoOvojSplit)
					* (previousSumWXXR[k] - (2 * previousSumWXR[k] * (previousSumXR[k] / vkupenBrojElementiVoOvojSplit))
							+ (((previousSumXR[k] / vkupenBrojElementiVoOvojSplit)
									* (previousSumXR[k] / vkupenBrojElementiVoOvojSplit)) * previousSumWR[k]));
			denR[k] = (previousSumWR[k]
					* (previousSumX2R[k] - (((previousSumXR[k] * previousSumXR[k])) / vkupenBrojElementiVoOvojSplit)));
			if (denR[k] != 0 && numR[k] != 0 && !flagRightAllEqual)
				ikkR[k] = numR[k] / denR[k]; // I for each target
			else
	...
	// global I
	@Override
	public double calcItotal(Integer[] permutation) throws ClusException { // matejp: all indices of tuples must be
																			// replaced by the corresponding
																			// permutation[tuple index]
		ClusSchema schema = m_data.getSchema();
		int M = 0;
		int N = m_data.getNbRows();
		long NR = m_data.getNbRows();
