...
public class M5PrunerMulti extends PruneTree {
    ...
    public boolean allAccurate(RegressionStat stat) {
        for (int i = 0; i < stat.getNbAttributes(); i++) {
            double E_leaf = stat.getRootScaledVariance(i, m_TargetWeights) * pruningFactor(stat.getTotalWeight(), 1);
            if (E_leaf >= m_GlobalRMSE[i] * m_F)
                return false;
        }
        return true;
    }
    ...
    public boolean allBetterThanTree(ClusNode node, RegressionStat stat, int modelsize) {
        for (int i = 0; i < stat.getNbAttributes(); i++) {
            double E_leaf = stat.getRootScaledVariance(i, m_TargetWeights) * pruningFactor(stat.getTotalWeight(), 1);
            double E_tree = estimateRootScaledVariance(node, i, m_TargetWeights) * pruningFactor(stat.getTotalWeight(), modelsize);
            if (E_leaf > E_tree) { return false; }
        }
        return true;
    }
