...
package clus.util.jeans.io;

import java.io.PrintWriter;

import clus.util.jeans.util.IntegerStack;
...
public class TabbedPrinter {
    ...
    public void printTab(String strg) {
        ...
        int len = strg.length();
        int max = tab != -1 ? Math.max(tab - m_linePos - 1, 0) : Math.max(m_lineSize - m_linePos - 1, 0);
        if (max < len)
            print(strg.substring(0, max) + "\t");
        else
            print(strg + "\t");
    }
