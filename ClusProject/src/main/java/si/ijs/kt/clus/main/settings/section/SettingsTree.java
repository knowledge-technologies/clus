...
public class SettingsTree extends SettingsBase {
	...
	public INIFileSection create() {
		...
		m_SectionTree.addNode(m_Heuristic = new INIFileEnum<>("Heuristic", Heuristic.Default));

		m_SectionTree.addNode(
				m_HeuristicComplexity = new INIFileEnum<>("HeuristicComplexity", TimeSeriesPrototypeComplexity.N2));
		m_SectionTree.addNode(m_SetDistance = new INIFileEnum<>("SetDistance", SetDistance.GSMDistance));
		m_SectionTree.addNode(m_TupleDistance = new INIFileEnum<>("TupleDistance", TupleDistance.Euclidean));
		m_SectionTree.addNode(m_TSDistance = new INIFileEnum<>("TSDistance", TimeSeriesDistanceMeasure.DTW));

		m_SectionTree.addNode(m_PruningMethod = new INIFileEnum<>("PruningMethod", PruningMethod.Default));
		...
		// added by Jurica Levatic, JSI
		m_SectionTree.addNode(m_MissingClusteringAttrHandling = new INIFileEnum<>("MissingClusteringAttrHandling",
				MissingClusteringAttributeHandlingType.EstimateFromParentNode));
		m_SectionTree.addNode(m_MissingTargetAttrHandling = new INIFileEnum<>("MissingTargetAttrHandling",
				MissingTargetAttributeHandlingType.ParentNode));

		// added by Eduardo Costa 06/06/2011
		m_SectionTree.addNode(m_InductionOrder = new INIFileEnum<>("InductionOrder", InductionOrder.DepthFirst));

		// added by Eduardo Costa 29/09/2011
		m_SectionTree.addNode(m_EntropyType = new INIFileEnum<>("EntropyType", EntropyType.StandardEntropy));
		m_SectionTree.addNode(
				m_ConsiderUnlableInstancesInIGCalc = new INIFileBool("ConsiderUnlableInstancesInIGCalc", false));

		m_SectionTree.addNode(m_SpatialMatrix = new INIFileEnum<>("SpatialMatrix", SpatialMatrixType.Binary));
		m_SectionTree.addNode(m_SpatialMeasure = new INIFileEnum<>("SpatialMeasure", SpatialMeasure.GlobalMoran));
		...
		m_SectionTree.addNode(m_SplitPosition = new INIFileEnum<>("SplitPosition", SplitPositions.Exact));
