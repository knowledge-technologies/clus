...
package si.ijs.kt.clus.util.jeans.graph.drawable;
...
public class DrawableLine extends Drawable {
    ...
    public void draw(DrawableProvider prov, int xofs, int yofs) {
        ...
        switch (xorient) {
            case CENTER:
                xpos = xpos + wd / 2 - fm.stringWidth(line) / 2;
                break;
            case UPPER:
                xpos = xpos + wd - fm.stringWidth(line);
                break;
        }
        switch (yorient) {
            case CENTER:
                ypos = ypos + hi / 2 - getHeight(fm) / 2;
                break;
            case UPPER:
                ypos = ypos + hi - getHeight(fm);
                break;
        }
