...
public class ClusModelInfo implements Serializable {
    ...
    public void setAllErrorsClone(ClusErrorList train, ClusErrorList test, ClusErrorList valid) {
        ...
        m_ValidErr = null;
        if (train != null)
            m_TrainErr = train.getErrorClone();
        if (test != null)
            m_TestErr = test.getErrorClone();
        if (valid != null)
            m_ValidErr = valid.getErrorClone();
    }
    ...
    public void setSelectedErrorsClone(ClusErrorList train, ClusErrorList test, ClusErrorList valid) {
        ...
        m_ValidErr = null;
        if (train != null)
            m_TrainErr = train.getErrorClone(getName());
        if (test != null)
            m_TestErr = test.getErrorClone(getName());
        if (valid != null)
            m_ValidErr = valid.getErrorClone(getName());
    }
    ...
    public final void addEnsemblePredictionWriter(int type, ClusEnsemblePredictionWriter wrtr) {
        if (type == ClusModelInfo.TEST_ERR)
            m_TestPreds = wrtr;
    ...
    public final ModelProcessorCollection getAddModelProcessors(int type) {
        if (type == TRAIN_ERR) {
            if (m_TrainModelProc == null)
                m_TrainModelProc = new ModelProcessorCollection();
            return m_TrainModelProc;
        }
        else if (type == TEST_ERR) {
            if (m_TestModelProc == null)
                m_TestModelProc = new ModelProcessorCollection();
            return m_TestModelProc;
        }
        else {
            if (m_ValidModelProc == null)
                m_ValidModelProc = new ModelProcessorCollection();
    ...
    public final void initModelProcessors(int type, ClusSchema schema) throws IOException, ClusException {
        ...
        if (coll != null)
            coll.initialize(m_Model, schema);
    }
    ...
    public final void terminateEnsemblePredictionWriter(int type) {
        if (type == TEST_ERR)
            m_TestPreds.closeWriter();
        if (type == TRAIN_ERR)
            m_TrainPreds.closeWriter();
    }
    ...
    public final ClusErrorList getError(int traintest) {
        if (traintest == TRAIN_ERR)
            return m_TrainErr;
        else if (traintest == VALID_ERR)
            return m_ValidErr;
        else if (traintest == TEST_ERR)
            return m_TestErr;
        else
            return null;
    }
    ...
    public final void updateName(ClusModelInfo other) throws ClusException {
        if (hasName()) {
            if (other.hasName() && !getName().equals(other.getName())) { throw new ClusException("Combining error measures of different models: " + getName() + " <> " + other.getName()); }
        }
        else {
            if (other.hasName())
