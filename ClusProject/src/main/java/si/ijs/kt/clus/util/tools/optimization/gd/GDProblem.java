...
/**
 * Class representing a gradient descent optimization problem. This gives tool
 * functions for the actual optimization algorithm.
 * 
 * @author Timo Aho
 */
public class GDProblem extends OptimizationProblem {
	...
	/**
	 * Is covariance computed for this dimension? To reduce computational costs,
	 ...
	 * times on the same predictions etc.
	 */
	...
	/**
	 * Number of rules with nonzero weights. I.e. how many trues in
	 * m_isWeightNonZero. The 'default rule' - the first rule that is always a
	 * constant is not counted!
	 */
	 ...
	 * computing gradients from scratch.
	 */
	protected double[] m_predCovWithTrue;

	/**
	 * Computed negative gradient averages for each of the dimensions [iWeight]
	 */
	...
	// // The following are for efficient step size computation if dynamic step size
	// is used
	...
	/**
	 * Constructor for problem to be solved with gradient descent. Currently only
	 * for regression.
	 * 
	...
	public GDProblem(ClusStatManager stat_mgr, OptimizationParameter optInfo, ClusRuleSet rset) {
		...
		// If needed, prepares for norm. Has to be done after normalization computation
		// and before covariance computing.
		// May be safe to do this before splitting validation set (if default rule is
		// used stored/used in validation set
		// by mistake).
		preparePredictionsForNormalization();

		// If early stopping criteria is chosen, reserve part of the training set for
		// early stop testing.
		if (getSettings().getRules().getOptGDEarlyStopAmount() > 0) {
			...
			OptimizationParameter dataEarlyStop = new OptimizationParameter(optInfo.m_rulePredictions.length,
					optInfo.m_baseFuncPredictions.length, nbDataTest, getNbOfTargets(), optInfo.m_implicitLinearTerms);
			OptimizationParameter trainingSet = new OptimizationParameter(optInfo.m_rulePredictions.length,
					optInfo.m_baseFuncPredictions.length, getNbOfInstances() - nbDataTest, getNbOfTargets(),
					optInfo.m_implicitLinearTerms);
		...
		for (int i = 0; i < nbWeights; i++) {
			Arrays.setAll(m_covariances[i], (idx) -> Double.NaN);
		}
	...
	private void computeDynStepSize() {
		...
		// m_dynStepSizeDrop = Math.pow(m_dynStepLowerBound,
		// 1.0/(getSettings().getOptGDMaxIter()/1000.0-1)); // first
	...
	/**
	 * Compute covariances between predictors and true values. With all zero weight
	 * vector these are also initial gradients before first iteration.
	 */
	...
	/**
	 * Estimate of expected value of covariance for given prediction. The covariance
	 * of this prediction with its true value is returned.
	 */
	...
	private void computeWeightCov(int dimension) {
		...
		// phase b was dimension
		// Thus if covariances for b are computed, this does not have to be computed
		// anymore.

		// The weights with index lower than this
		for (int iMin = 0; iMin < dimension; iMin++) {
			// If the covariances for the other part are already computed, this covariance
			// is already
			// computed also. Thus skip this covariance.
			if (!m_isCovComputed[iMin]) {
				// if (!Double.isNaN(m_covariances[iMin][dimension][0]))
				// System.err.println("WARNING: Covariances are recalculated, waste of
				// computation!");
	...
	/**
	 * Compute covariance between two predictions (base learner predictions). This
	 * is a help function for computeWeightCov NOTE: Second parameter index HAS to
	...
	private double computeCovForRuleAndLin(int iRule, int iLinear) {
		...
		for (int iInstance = getRuleNextCovered(iRule, 0); iInstance >= 0; iInstance = getRuleNextCovered(iRule,
				iInstance + 1)) {
	...
	private int getRuleEnabledTargets(int iRule) {
		...
		if (getSettings().getEnsemble().isEnsembleROSEnabled() && getSettings().getEnsemble().getEnsembleROSVotingType()
				.equals(EnsembleROSVotingType.SubspaceAveraging)) {
	...
	/**
	 * Returns the real prediction when this rule is used. If the rule does not give
	...
	final protected double predictWithRule(int iRule, int iInstance, int iTarget) {
		// return isCovered(iRule, iInstance) ? getPredictionsWhenCovered(iRule,
		// iInstance, iTarget) : 0d;
	...
	public void fullGradientComputation(ArrayList<Double> weights) {
		...
		// computeDynamicStepSizeComputation();
	}
	...
	/**
	 * Recompute the gradients new iteration. This is lot of faster than computing
	...
	public void modifyGradientSquared(int[] iChangedWeights) {
		...
		// Could store only the changes to the gradients and do the actual change
		// afterwards in another for loop
		...
		/**
		 * If we reach linear terms with the first loop, we can skip most of the terms
		 * in the second loop (because cov is 0)
		 */
			...
			for (int iWeightChange = 0; iWeightChange < nbOfGrads; iWeightChange++) {
				...
				if (firstLinearTermReached) {
					if (secondLinearTermReached) {
						// Skip to only those linear terms, for which
						// cov(iWeightChange,iChangedWeights[iiAffecting])!=
						// 0
						...
						// This is linear term number linTargetDim(). However this is already the first
						// linear term and
						// there is already
	...
	/**
	 * Return the gradients with maximum absolute value. For the weights we want to
	 * change The function is assuming (if max allowed rule nb is set) that index 0
	 * of gradients includes the "default rule" which is not counted.
	 * 
	 * @param nbOfIterations Used for detecting if weight is banned (not usually
	 *                       used)
	 */
	public int[] getMaxGradients(int nbOfIterations) {
		...
		for (int iCopy = 0; iCopy < m_gradients.length; iCopy++) {
			...
			if (Math.abs(m_gradients[iCopy]) >= minAllowed && (!maxNbOfWeightReached || m_isWeightNonZero[iCopy])
					|| iCopy == 0) {
				...
				for (int iGrad = 0; iGrad < iMaxGradients.size(); iGrad++) {
					...
					if (!m_isWeightNonZero[iMaxGradients.get(iGrad)] && iMaxGradients.get(iGrad) != 0) {
						...
						while (iAllowed.hasNext()) {
							...
							if (Math.abs(m_gradients[iAllowed.next()]) < Math
									.abs(m_gradients[iMaxGradients.get(iGrad)])) {
								iAllowed.previous();
								break;
							}
						}
	...
	/**
	 * Compute the needed covariances for the weight. Only called if we are going to
	 * change the weight.
	 */
	...
	/**
	 * In case of oscillation, make the step size shorter We should be changing the
	 * step size just enough not to prevent further oscillation
	 */
	final public void dropStepSize(double amount) {
		...
		// m_stepSize *= 0.1;
		m_stepSize *= amount; // We make the new step size a little smaller than is limit (because of rounding
								// mistakes)
	...
	/**
	 * Early stopping is triggered when the error rate is bigger than the
	 * smallest_error_rate multiplied by
	 * getSettings().getRules().getOptGDEarlyStopTreshold() = overfitting
	 */

	public boolean isEarlyStop(ArrayList<Double> weights) {
		double newFitness = m_earlyStopProbl.calcFitness(weights, m_ruleSet);
	...
	/**
	 * Return a maximum tree depth based on RulefFit (Friedman, Popescu. 2005)
	 * function used. Function is Pr(nbOfLeaves) =
	 * exp(-nbOfLeaves/(avgDepth/2))/(avgDepth-2).
	 * 
	...
	public static int randDepthWighExponentialDistribution(double unifRand, int avgDepth) {
		// Friedman is computing the number of terminal leaves with exponential
		// distribution
		// To get the limit depth for this we need to compute
		// depth = ceil(log(2+floor(-(L-2)/(lg(L-2))lg(unifRand)))) +1
		// Here L is the average and 2+floor(-(L-2)/(lg(L-2))lg(unifRand)) is the number
		// of terminal leaves
		// However in Clus depth seems to be so that only root is on depth 0, and depth
		// 1 has two leaves. Thus
		// we leave the last +1 out and compute
		// ceil(log(2+floor(-(L-2)/(lg(L-2))lg(unifRand))))
		// Also we do not take the floor for computing amount of terminal nodes because
		// this reduces the average
		// depth too much. Without it the average depth for value 3 is in reality 2.7
		int maxDepths = 0;

		if (unifRand == 0.0) {
			// This means that the limit depth should be infinite. Can't take logarithm of
			// this
			maxDepths = -1;
		} else {
