...
public class DrawableCanvas extends JComponent {
    ...
    public Point getPosition(Drawable drawable, int xp, int yp) {
        ...
        if (xpos + rect.width + 3 > pt.x)
            xpos = pt.x - rect.width - 3;
        if (ypos + rect.height + 3 > pt.y)
            ypos = pt.y - rect.height - 3;
    ...
    public void addDrawable(Drawable drawable) {
        if (m_Transform != null)
            m_Transform.transform(drawable);
        Rectangle rect = drawable.getBoundRect();
        int x2 = rect.x + rect.width;
        int y2 = rect.y + rect.height;
        // Calculate rect's
    ...
    public Dimension transformDimension(Dimension dim) {
        if (m_Transform == null)
            ...
            return m_Transform.transformDimension(dim);
    }
    ...
    public void render(Graphics2D g) {
        if (renderer != null) {
            ...
            if (m_Rendered != null)
                m_Rendered.actionPerformed(null);
        }
        else {
            clear();
        }
    ...
    public void paintComponent(Graphics g) {
        ...
        if (renderstate != STATE_RENDERERD)
            render(g2);
