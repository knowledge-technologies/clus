...
public class SettingsEnsemble extends SettingsBase {
	...
	public int calculateNbRandomAttrSelected(ClusSchema schema, RandomAttributeTypeSelection type) {
		...
		if (value.equalsIgnoreCase("SQRT")) {
			...
			try {
				...
				if (val > ubound) {
					ClusLogger
					        .severe("The size of the subset can't be larger than the number of " + s + " attributes.");
				...
				try {
					...
					ClusLogger.severe(
					        "\t RANDOM for taking a random subset of targets (works only in ensemble mode for target subspacing).");
	...
	public void create() {
		...
		m_Section.addNode(
		        m_EnsembleBootstrapping = new INIFileEnum<>("EnsembleBootstrapping", EnsembleBootstrapping.Undefined));
		m_Section.addNode(
		        m_ClassificationVoteType = new INIFileEnum<>("VotingType", VotingType.ProbabilityDistribution));
		...
		m_Section.addNode(
		        m_EnsembleROSAlgorithmType = new INIFileEnum<>("ROSAlgorithmType", EnsembleROSAlgorithmType.Disabled));
		m_Section.addNode(
		        m_EnsembleROSVotingType = new INIFileEnum<>("ROSVotingType", EnsembleROSVotingType.SubspaceAveraging));
	...
	public List<String> validateSettingsInternal() {
		...
		validROSCombinations.put(EnsembleROSAlgorithmType.FixedSubspaces,
		        Arrays.asList(EnsembleROSVotingType.TotalAveraging, EnsembleROSVotingType.SubspaceAveraging));
		validROSCombinations.put(EnsembleROSAlgorithmType.DynamicSubspaces,
		        Arrays.asList(EnsembleROSVotingType.TotalAveraging));
		...
		/* combination of ROS algorithm and prediction algorithm is not valid */
			...
			incompatible.add(formatInvalid(m_EnsembleROSAlgorithmType, getEnsembleROSAlgorithmType(), String.format(
			        "Cannot be used with %s = %s", m_EnsembleROSVotingType.getName(), getEnsembleROSVotingType())));
		}
		...
		if (isEnsembleMode()) {
			...
			if (!r.equals(EnsembleRanking.None)) {
				if ((!m.equals(EnsembleMethod.Bagging) && !m.equals(EnsembleMethod.RForest)
				        && !m.equals(EnsembleMethod.ExtraTrees))
