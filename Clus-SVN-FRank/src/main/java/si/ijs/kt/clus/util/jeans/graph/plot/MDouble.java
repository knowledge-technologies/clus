...
package si.ijs.kt.clus.util.jeans.graph.plot;
...
public class MDouble {
    ...
    public static void killZeros(StringBuffer buffer) {
        ...
        int pos = len - 1;
        while (pos > 0 && buffer.charAt(pos) == '0')
            pos--;
        if (pos != len - 1) {
    ...
    public static int getNbDigits(double number) {
        ...
        if (val == 0.0f)
            return 0;
        if (val >= 1.0f)
            return (int) Math.floor(Math.log(val) / Math.log(10f) + 1e-7) + 1;
        else
            return -(int) Math.ceil(-Math.log(val) / Math.log(10f) - 1e-7);
    }
