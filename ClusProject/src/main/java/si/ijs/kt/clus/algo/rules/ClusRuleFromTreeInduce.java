...
/**
 * Create rules by decision tree ensemble algorithms (forests). Use this by
 * 'CoveringMethod = RulesFromTree' .
 *
 * This has to be own induce class because we need Clus instance for creating
 * tree ensemble.
 * 
 * @author Timo Aho
 *
...
public class ClusRuleFromTreeInduce extends ClusRuleInduce {
	...
	public ClusModel induceSingleUnpruned(ClusRun cr) throws Exception {
		...
		/**
		 * The params may already have been disabled, thus we do not want to disable
		 * them again (forgets original values) getSettings().returnRuleInduceParams();
		 */
		...
		/**
		 * The real trained ensemble model without pruning. Use unpruned tree because
		 * weight optimizing should get rid of bad rules anyway.
		 */
		ClusForest forestModel = (ClusForest) cr.getModel(ClusModel.ORIGINAL);

		/**
		 * The class for transforming single trees to rules Parameter always true
		 */
		...
		ClusLogger.info("Transformed " + forestModel.getNbModels() + " trees in ensemble into rules. Created "
				+ ruleSet.getModelSize() + " rules. (" + numberOfUniqueRules + " of them are unique.)");

		// The default rule, which will later be explicitly added to the rule set (i.e.,
		// will be equal to other rules)
