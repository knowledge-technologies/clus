...
public class ClusRuleSet implements ClusModel, Serializable {
    ...
    public void printModel(PrintWriter wrt, StatisticPrintInfo info) {
        ...
        if (getSettings().getRules().getRulePredictionMethod().equals(RulePredictionMethod.GDOptimized) && getSettings().getRules().getOptGDNbOfTParameterTry() > 1) {
            ...
            wrt.println("Gradient descent optimization: Smallest test fitness of " + fr.format(m_optWeightBestFitness) + " with T value: " + fr.format(m_optWeightBestTValue) + System.lineSeparator());
        }
    ...
    public int addRuleSet(ClusRuleSet newRules, boolean addOnlyUnique) {
        ...
        for (int iRule = 0; iRule < newRules.getModelSize(); iRule++) {
            ...
            if (addOnlyUnique) {
                ...
                /** if covering method = sampling we are only interested in descriptive part */
                if ((getSettings().getRules().isRulePredictionOptimized() && getSettings().getRules().isOptOmitRulePredictions()) || getSettings().getRules().getCoveringMethod().equals(CoveringMethod.SampledRuleSet)) {
