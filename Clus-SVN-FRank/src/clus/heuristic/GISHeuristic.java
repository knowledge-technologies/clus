    ...
    public double calcI(ClusStatistic t_stat, ClusStatistic p_stat, ClusStatistic missing, Integer[] permutation) {
        ...
        ClusStatistic pstat = (ClusStatistic)p_stat;
          ...
          if (stopCriterion(t_stat, p_stat, missing)) {
            ...
            case 4:  ss_pos = pstat.calcGLocalTotal();  break; //"Local Geary"
            case 5:  ss_pos = pstat.calcLocalGetisTotal(); break;//"Local Getis"
            case 6:  ss_pos = pstat.calcGETIStotal(); break;//Local Standardized Getis
            case 7:  ss_pos = pstat.calcEquvalentItotal();break;//EquvalentI 
            case 8:  ss_pos = pstat.calcIwithNeighbourstotal(); break; //Global Moran with neigh
            case 9:  ss_pos = pstat.calcEquvalentIwithNeighbourstotal(); break; // EquvalentI Global Moran with neigh
            case 10:  ss_pos = pstat.calcItotalD(); break; //"Global Moran with a separate distance file"
            case 11:  ss_pos = pstat.calcGtotalD();  break; //"Global Geary with a separate distance file"
            case 12:  ss_pos = pstat.calcCItotal(); break; // Connectivity Index (CI) for Graph Data
            case 13:  ss_pos = pstat.calcMutivariateItotal(); break; //Cross Moran (Wartenberg, 1985)
            case 14:  ss_pos = pstat.calcCwithNeighbourstotal(); break; //Global Geary with neigh
            case 15:  ss_pos = pstat.calcBivariateLee(); break; //Bivariate Lee's measure: integration of Moran&Pearson coef. (Lee, 2001)
            case 16:  ss_pos = pstat.calcMultiIwithNeighbours(); break; //Cross Moran (Wartenberg, 1985) with neigh
            case 17:  ss_pos = pstat.calcCIwithNeighbours(); break; // Connectivity Index (CI) for Graph Data with neigh
            case 18:  ss_pos = pstat.calcLeewithNeighbours(); break; //Bivariate Lee's measure with neigh
            case 19:  ss_pos = pstat.calcPtotal(); break; //Global Moran without weights i.e Pearson c. coef.
            case 20:  ss_pos = pstat.calcCItotalD(); break; // Connectivity Index (CI) for Graph Data with a separate distance file
            case 21:  ss_pos = pstat.calcDHtotalD(); break; // Dyadicity and Heterophilicity for Graph Data with a separate distance file
            case 22:  ss_pos = pstat.calcEquvalentIDistance(); break; // EquvalentI with a separate distance file
            case 23:  ss_pos = pstat.calcPDistance(); break; //Pearson c. coef. with a separate distance file
            case 24:  ss_pos = pstat.calcEquvalentGtotal(); break; //EquvalentG, (Global Geary)
            case 25:  ss_pos = pstat.calcEquvalentGDistance(); break; //EquvalentGDistance, EquvalentG with a separate distance file
            case 26:  ss_pos = pstat.calcEquvalentPDistance(); break; //EquvalentGDistance, EquvalentP with a separate distance file
