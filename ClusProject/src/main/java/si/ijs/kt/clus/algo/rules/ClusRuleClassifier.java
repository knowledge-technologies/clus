...
public class ClusRuleClassifier extends ClusInductionAlgorithmType {
	...
	public ClusInductionAlgorithm createInduce(ClusSchema schema, Settings sett, CMDLineArgs cargs)
		...
		// Compute the normalization information here if needed. We can here use the
		// whole data set
	...
	public void printInfo() {
		if (getSettings().getRules().getCoveringMethod().equals(CoveringMethod.SampledRuleSet)) {
			ClusLogger.info("RuleSystem based on sampling from initial set of rules generated by: "
					+ getSettings().getRules().getInitialRuleGeneratingMethod().toString());
		} else if (getSettings().getRules().isRandomRules()) {
			ClusLogger.info("RuleSystem generating random rules");
		} else if (getSettings().getRules().getCoveringMethod().equals(CoveringMethod.RulesFromTree)
				&& getSettings().getRules().getRulePredictionMethod().equals(RulePredictionMethod.GDOptimized)) {
	...
	public void postProcess(ClusRun cr) throws ClusException, IOException, InterruptedException {
		...
		if (!getSettings().getRules().getCoveringMethod().equals(CoveringMethod.RulesFromTree)
				&& !getSettings().getRules().getCoveringMethod().equals(CoveringMethod.SampledRuleSet)) {
