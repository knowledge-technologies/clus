...
package clus.util.jeans.io.ini;

import java.io.IOException;
import java.io.StringReader;
import java.util.ArrayList;

import clus.util.jeans.io.range.NominalType;
        ...
        if (tokens.isNextToken('[')) {
            ...
            if (idx != -1) {
                ...
                for (int i = 0; i < m_Names.length; i++) {
                    if (!isArrayIndexName(i, token)) { throw new IOException("Expected one of [" + getArrayIndexNamesString() + "] and not '" + token + "' for setting '" + getName() + "'"); }
                    String value = tokens.getToken();
                    if (!value.equals("=")) { throw new IOException("Expected '=' after '" + m_Names[i] + "' for setting '" + getName() + "'"); }
    ...
    public String getStringValue() {
        if (hasArrayIndexNames()) {
            ...
            for (int i = 0; i < m_Names.length; i++) {
                ...
                if (isNominal(i))
                    buf.append(getNominalString(i));
                else
                    buf.append(String.valueOf(getDouble(i)));
                if (i != m_Names.length - 1)
                    buf.append("\n");
            }
            return buf.toString();
            ...
            if (isNominal())
                return m_Type.getName(getNominal());
            else
