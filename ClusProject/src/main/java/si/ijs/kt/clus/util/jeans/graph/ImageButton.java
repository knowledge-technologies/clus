...
package si.ijs.kt.clus.util.jeans.graph;
...
import si.ijs.kt.clus.util.jeans.graph.image.GrayScaleFilter;
 ...
 * the button is sized when the image size is known.
 * <p>
 *
...
public class ImageButton extends Canvas {
     ...
     *        the button image
     * @param label
     *        the button label (used in action events)
     * @param shadow
     *        the button shadow width
     * @param border
     *        the button border width
     * @param resizeImage
     *        true if image to be resized to actual width
     *        of button.
     * @param showLabel
     *        if label to be displayed as well as image.
     * @param minWidth
     *        minimum width (pixels). Useful if you want
     ...
     * 
     * @param label
     *        the label to set the button with
     * @see #getLabel
     */
     ...
     * 
     * @param image
     *        the image to set the button with
     * @see #getImage
     */
     ...
     * 
     * @param showLabel
     *        true if label to be displayed as well as image.
     */
    ...
    public synchronized Dimension getMinimumSize() {
        ...
        if (image == null || showLabel) {
            ...
            labelDimension.width = Math.max(fm.stringWidth(label) + 2 * (shadow + border), minWidth);
            labelDimension.height = fm.getAscent() + fm.getDescent() + 2 * (shadow + border);
            if (image == null)
                d = labelDimension;
        }
        if (image != null) {
