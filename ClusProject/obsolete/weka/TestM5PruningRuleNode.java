 ...
 * does not have the required set methods.
 */
...
public class TestM5PruningRuleNode extends Classifier {
    ...
    public static void createM5RuleNodeRecursive(TestM5PruningRuleNode result, ClusNode node, RowData data, ClusToWekaData cnv) {
        result.m_classIndex = cnv.getClassIndex();
        result.m_instances = cnv.convertData(data);
        result.m_isLeaf = node.atBottomLevel();
        result.m_numInstances = data.getNbRows();
        result.m_numAttributes = result.m_instances.numAttributes();
        result.m_regressionTree = true;
        result.m_indices = new int[1];
        result.m_indices[0] = cnv.getClassIndex();
        result.m_numParameters = 1;
        NodeTest tst = node.getTest();
        if (tst != null) {
            result.m_splitAtt = cnv.getIndex(tst.getType().getName());
            if (tst instanceof NumericTest)
                result.m_splitValue = ((NumericTest) tst).getBound();
        }
        if (node.getNbChildren() != 0) {
            for (int i = 0; i < 2; i++) {
                TestM5PruningRuleNode child = new TestM5PruningRuleNode(result.m_globalDeviation, result.m_globalAbsDeviation, result);
                RowData subset = data.applyWeighted(tst, i);
                createM5RuleNodeRecursive(child, (ClusNode) node.getChild(i), subset, cnv);
                if (i == 0)
                    result.m_left = child;
                else
                    result.m_right = child;
            }
        }
    }
    ...
    public static TestM5PruningRuleNode createM5RuleNode(ClusNode node, RowData data, ClusToWekaData cnv) {
        Instances inst = cnv.convertData(data);
        double gstddev = stdDev(cnv.getClassIndex(), inst);
        double gabsdev = absDev(cnv.getClassIndex(), inst);
        TestM5PruningRuleNode result = new TestM5PruningRuleNode(gstddev, gabsdev, null);
        createM5RuleNodeRecursive(result, node, data, cnv);
        return result;
    }
    ...
    /**
     * Creates a new <code>TestM5PruningRuleNode</code> instance.
     *
     * @param globalDev
    ...
    public void buildClassifier(Instances data) throws Exception {
        ...
        if ((m_numInstances < m_splitNum) || (stdDev(m_classIndex, m_instances) < (m_globalDeviation * m_devFraction))) {
            m_isLeaf = true;
        }
        else {
            m_isLeaf = false;
    ...
    public double classifyInstance(Instance inst) throws Exception {
        if (m_isLeaf) {
            if (m_nodeModel == null) { throw new Exception("Classifier has not been built correctly."); }
    ...
    public void split() throws Exception {
        ...
        if (!m_isLeaf) {
            ...
            if (bestSplit.splitAttr() < 0 || bestSplit.position() < 1 || bestSplit.position() > m_numInstances - 1) {
                ...
                m_left = new TestM5PruningRuleNode(m_globalDeviation, m_globalAbsDeviation, this);
                ...
                m_right = new TestM5PruningRuleNode(m_globalDeviation, m_globalAbsDeviation, this);
    ...
    public String treeToString(int level) {
        ...
        if (!m_isLeaf) {
            ...
            if (m_globalDeviation > 0.0) {
                text.append(" (" + m_numInstances + "/" + Utils.doubleToString((100.0 * m_rootMeanSquaredError / m_globalAbsDeviation), 1, 3) + "%)\n");
            }
            else {
    ...
    public void installSmoothedModels() throws Exception {
        ...
        if (m_isLeaf) {
            ...
            TestM5PruningRuleNode current = this;
    ...
    private double pruningFactor(int num_instances, int num_params) {
        ...
        return ((double) (num_instances + m_pruningMultiplier * num_params) / (double) (num_instances - num_params));
    }
    ...
    protected void graphTree(StringBuffer text) {
        text.append("N" + m_id + (m_isLeaf ? " [label=\"LM " + m_leafModelNum : " [label=\"" + m_instances.attribute(m_splitAtt).name()) + (m_isLeaf ? " (" + ((m_globalDeviation > 0.0) ? m_numInstances + "/" + Utils.doubleToString((100.0 * m_rootMeanSquaredError / m_globalAbsDeviation), 1, 3) + "%)" : m_numInstances + ")") + "\" shape=box style=filled " : "\"") + (m_saveInstances ? "data=\n" + m_instances + "\n,\n" : "") + "]\n");
