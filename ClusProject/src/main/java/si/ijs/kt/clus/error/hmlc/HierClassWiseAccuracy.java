...
public class HierClassWiseAccuracy extends ClusError {
    ...
    public void addExample(DataTuple tuple, ClusStatistic pred) {
        // ClusLogger.info("tuple: "+tuple.toString());
        ...
        for (int i = 0; i < m_Dim; i++) {
            if (predarr[i]) {
                ...
                // ClusLogger.info("Ex: "+tuple.toString());
                // ClusLogger.info(pred.getClassString()+" "+m_Hier.getTermAt(i).toStringHuman(m_Hier));
    ...
    public boolean isEvalClass(int idx) {
        if (isNoTrivialClasses()) {
            ...
            if (m_NbPosActual[idx] == getNbTotal()) { return false; }
        }
