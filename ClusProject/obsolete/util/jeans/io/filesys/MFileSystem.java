...
package si.ijs.kt.clus.util.jeans.io.filesys;
...
import si.ijs.kt.clus.util.jeans.util.sort.MStringSorter;
import si.ijs.kt.clus.util.jeans.util.sort.MVectorSorter;
import si.ijs.kt.clus.util.jeans.util.thread.MCallback;
import si.ijs.kt.clus.util.jeans.util.thread.MWorkerThread;
...
public abstract class MFileSystem {
    ...
    public String getDirectoryAt(int idx) {
        ...
        if (idx < curr.size())
            return (String) curr.elementAt(idx);
        else
            return "";
    }
    ...
    public MFileEntry getFileAt(int idx) {
        ...
        if (idx < curr.size())
            return (MFileEntry) curr.elementAt(idx);
        else
            return null;
    }
    ...
    public boolean reload(int type) {
        ...
        if (m_hWorker == null)
            m_hWorker = MWorkerThread.getInstance();
    ...
    public class MyWorker implements Runnable {
        ...
        public void run() {
            ...
            if (m_hCallback != null)
                m_hCallback.callBack(result, m_iType);
        }
    }
