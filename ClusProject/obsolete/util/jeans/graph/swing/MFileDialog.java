...
package si.ijs.kt.clus.util.jeans.graph.swing;
...
import si.ijs.kt.clus.util.jeans.graph.PercentLayout;
import si.ijs.kt.clus.util.jeans.graph.WindowClosingListener;
import si.ijs.kt.clus.util.jeans.io.filesys.MFileEntry;
import si.ijs.kt.clus.util.jeans.io.filesys.MFileSystem;
import si.ijs.kt.clus.util.jeans.resource.MediaCache;
import si.ijs.kt.clus.util.jeans.util.thread.MCallback;
...
public class MFileDialog extends JDialog implements MCallback {
    ...
    public void setSingleSelection(boolean sel) {
        if (sel)
            m_hFileTable.setSelectionMode(ListSelectionModel.SINGLE_SELECTION);
        else
            m_hFileTable.setSelectionMode(ListSelectionModel.MULTIPLE_INTERVAL_SELECTION);
    }
    ...
    public class MyDirKeyListener extends KeyAdapter {
        ...
        public void keyReleased(KeyEvent e) {
            ...
            if (Character.isLetterOrDigit((char) ch) || ch == '-' || ch == '_') {
                if (m_EntrySequence == null)
                    m_EntrySequence = String.valueOf((char) ch);
                else
                    m_EntrySequence += String.valueOf((char) ch);
    ...
    public class MyListModel extends AbstractListModel {
        ...
        public void fireDataChanged() {
            ...
            m_hDirList.setSelectedIndex(0);
            if (size < m_iPrevSize)
                fireIntervalRemoved(this, size, m_iPrevSize - 1);
            if (size > m_iPrevSize)
                fireIntervalAdded(this, m_iPrevSize, size - 1);
    ...
    public class MyTableModel extends AbstractTableModel {
        ...
        public Object getValueAt(int row, int col) {
            ...
            if (col == 3)
                return entry.getDateString();
            if (col == 4)
                return entry.getLengthString();
            return "";
        }
