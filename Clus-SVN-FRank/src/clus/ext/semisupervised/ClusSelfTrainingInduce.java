...
import clus.ext.hierarchical.thresholdcalibration.HierThresholdCalibration;
import clus.ext.hierarchical.thresholdcalibration.calibrateByLabelCardinality;
import clus.ext.semisupervised.confidence.classification.ClassesProbabilities;
import clus.ext.semisupervised.confidence.OracleScore;
import clus.ext.semisupervised.confidence.PredictionConfidence;
import clus.ext.semisupervised.confidence.RandomScore;
import clus.ext.semisupervised.confidence.regression.VarianceScore;
import clus.ext.semisupervised.confidence.regression.RForestProximities;
import clus.ext.semisupervised.utils.DoublesPair;
import clus.ext.semisupervised.utils.IndiceValuePair;
...
public class ClusSelfTrainingInduce extends ClusSemiSupervisedInduce {
    ...
    public void initialize(ClusSchema schema, Settings sett) {
        m_UnlabeledCriteria = Settings.getUnlabeledCriteria();
    ...
    public ClusModel induceSingleUnpruned(ClusRun cr) throws ClusException,
        ...
        switch (m_Mode) {
        case ClusStatManager.MODE_HIERARCHICAL:
        	errorType = "AUPRC"; break;
        case ClusStatManager.MODE_REGRESSION:
        	errorType = "RMSE"; break;
        case ClusStatManager.MODE_CLASSIFY:
        	errorType = "Accuracy"; break;
        default: errorType = "RMSE";
        }
        ...
        if (cr.getStatManager().getSettings().getConfidenceMeasure() == Settings.SSL_CONFIDENCE_MEASURE_RFPROXIMITIES) {
        ...
        while (unlabeledAdded > 0
                && !(m_StoppingCriteria == Settings.SSL_STOPPING_CRITERIA_ITERATIONS
            ...
            //train base model
            if (!(m_UnlabeledCriteria == Settings.SSL_UNLABELED_CRITERIA_EXHAUSTIVESEARCH) || first) {
            ...
            if (first) {
                ...
                writer = new PrintWriter(cr.getStatManager().getSettings().getAppName() + "_SelfTrainingErrors.csv", "UTF-8");
            ...
            if (m_StoppingCriteria == Settings.SSL_STOPPING_CRITERIA_AIRBAG) {
            ...
            if (m_UnlabeledCriteria == Settings.SSL_UNLABELED_CRITERIA_AUTOMATICOOB || m_UnlabeledCriteria == Settings.SSL_UNLABELED_CRITERIA_AUTOMATICOOBINITIAL) {
                ...
                switch (m_OOBErrorCalculation) {
                    case Settings.SSL_OOB_ERROR_CALCULATION_LABELED_ONLY: //calculate OOB error only on original labeled part
                        maxInd = origLabeledMax;
                        confidencesOOBErrors = new DoublesPair[origLabeledMax];
                        OOBErrors = new double[origLabeledMax];
                        break;
                    case Settings.SSL_OOB_ERROR_CALCULATION_ALL_DATA: //calculate OOB error on all examples in the training set, i.e., unlabeled examples added to the training set are also considered
                ...
                for (int i = 0; i < maxInd; i++) { //calculate OOB errors             
                    ...
                    if (m_Mode == ClusStatManager.MODE_HIERARCHICAL) {
                        error = new HierErrorMeasures(errListOOB, m_StatManager.getHier(), m_StatManager.getSettings().getRecallValues().getDoubleVector(), getSettings().getCompatibility(), Settings.HIERMEASURE_POOLED_AUPRC, m_StatManager.getSettings().isWriteCurves());
                    }
                ...
                if (m_UnlabeledCriteria == Settings.SSL_UNLABELED_CRITERIA_AUTOMATICOOBINITIAL) {
                    m_UnlabeledCriteria = Settings.SSL_UNLABELED_CRITERIA_THRESHOLD;
                }
            ...
            if (ClusStatManager.getMode() == ClusStatManager.MODE_HIERARCHICAL) {
                if (cr.getStatManager().getSettings().shouldCalibrateHmcThreshold()) {
                ...
                if(m_StatManager.getSettings().getConfidenceMeasure() == Settings.SSL_CONFIDENCE_MEASURE_ORACLE)
            ...
            if (m_UnlabeledCriteria == Settings.SSL_UNLABELED_CRITERIA_KPERCENTAGEMOSTAVERAGE) {
            ...
            if (m_UnlabeledCriteria == Settings.SSL_UNLABELED_CRITERIA_THRESHOLD || m_UnlabeledCriteria == Settings.SSL_UNLABELED_CRITERIA_AUTOMATICOOB) {
            ...
            if (m_UnlabeledCriteria == Settings.SSL_UNLABELED_CRITERIA_KMOSTCONFIDENT) {
            ...
            if (m_UnlabeledCriteria == Settings.SSL_UNLABELED_CRITERIA_KPERCENTAGEMOSTCONFIDENT) {
            ...
            if (m_UnlabeledCriteria == Settings.SSL_UNLABELED_CRITERIA_EXHAUSTIVESEARCH) {
                ...
                do {
                    ...
                    for (double threshold : m_ExhaustiveSearchThresholds) {
                        ...
                        if (examplesAddedPerThreshold > 0) {
                            ...
                            ClusError errOOB;
                            switch (m_OOBErrorCalculation) {
                                case Settings.SSL_OOB_ERROR_CALCULATION_LABELED_ONLY:
                                    errOOB = getOOBError(m_TrainingSet, origLabeledMax);
                                    break;
                                case Settings.SSL_OOB_ERROR_CALCULATION_ALL_DATA:
     ...
     * unlabeled set
     *
     * @param t Example which should be processed
     * @param index Index of the example in the unlabeled set
     */
