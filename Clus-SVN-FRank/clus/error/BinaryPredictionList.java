...
/**
 * This class could be used in any kind of classification setting (e.g., hierarchical multilabel) and basically stores the statistics,
 * which enables as to compute the number of TP, TN, FP and FN (T - true, F - false, P - positives, and N - negatives) for any given threshold.<p>
 * 
 * The class is used for constructing ROC- and PR-curves, where the threshold vary, hence it basically stores the triplets
 * 
 *  <p>{@code (probability(X), truth, count)}<p>
 *  
 *  where
 *  <ul>
 *  <li>{@code 0.0 <= probability(X) <= 1.0} denotes our estimate of probability of presence of some fixed label X (or positive class),</li>
...
public class BinaryPredictionList implements Serializable {
	...
	protected transient HashMap<DoubleBooleanCount, DoubleBooleanCount> m_ValueSet = new HashMap<DoubleBooleanCount, DoubleBooleanCount>(); // key:value pairs are (hash of) x: x
	/**
	 * Updates the binary-classification statistics.
	 * @param actual Ground truth, denotes the presence of a given label (or belonging to positive class)
	 * @param predicted Ranges between {@code 0} and {@code 1} and gives the estimate of probability that the given example is labelled by the label (or belonging to positive class)<p>
	 * 
	...
	/**
	 * Sorts the {@code jeans.util.compound.DoubleBoolean}s from the {@code m_ValueSet}. Used for ROC- and PR-curves construction.
	 */
	public void sort() {
		m_Values.clear();
