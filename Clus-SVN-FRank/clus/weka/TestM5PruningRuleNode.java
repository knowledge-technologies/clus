...
public class TestM5PruningRuleNode extends Classifier {
	...
	public static void createM5RuleNodeRecursive(TestM5PruningRuleNode result, ClusNode node, RowData data, ClusToWekaData cnv) {
		...
		if (tst != null) {
			...
			if (tst instanceof NumericTest) result.m_splitValue = ((NumericTest)tst).getBound();
		}
		if (node.getNbChildren() != 0) {
			for (int i = 0; i < 2; i++) {
				...
				if (i == 0) result.m_left = child;
				else result.m_right = child;
			}
		}
	}
...
//		System.out.println(tree.treeToString(0));
	...
	public void buildClassifier(Instances data) throws Exception {
		...
		if ((m_numInstances < m_splitNum)
				|| (stdDev(m_classIndex, m_instances)
