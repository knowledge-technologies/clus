...
public class TreeFrame extends JFrame {
    ...
    public DefaultMutableTreeNode getParentFNode(String[] path) {
        ...
        while (pos < path.length - 1) {
            ...
            if (!found) {
                int ipos = 0;
                while (ipos < curr.getChildCount() && cmpName(curr.getChildAt(ipos).toString(), pstr)) {
                    ipos++;
                }
    ...
    public DefaultMutableTreeNode addFile(String full, String ds) {
        ...
        int ipos = 0;
        while (ipos < parent.getChildCount() && cmpName(parent.getChildAt(ipos).toString(), chname)) {
            ipos++;
        }
    ...
    public void setTree(ClusNode root, ClusStatManager mgr) throws ClusException {
        ...
        if (mgr != null)
            m_TreePanel.setStatManager(mgr);
        showInfo(root);
    }
