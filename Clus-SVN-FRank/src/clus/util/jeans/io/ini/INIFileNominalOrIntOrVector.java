...
public class INIFileNominalOrIntOrVector extends INIFileEntry {
    ...
    public int[] getNominalVector() {
        if (isVector()) {
            int[] res = new int[m_ArrayNominal.length];
            System.arraycopy(m_ArrayNominal, 0, res, 0, m_ArrayNominal.length);
            return res;
        }
        else {
            int[] res = new int[1];
            res[0] = m_SingleNominal;
            return res;
        }
    }
        ...
        if (isVector()) {
            ...
            for (int i = 0; i < getVectorLength(); i++) {
                ...
                if (isNominal(i))
                    ...
                    buf.append(String.valueOf(getInt(i)));
            }
            buf.append("]");
            return buf.toString();
        }
