...
public class ClusNode extends MyNode implements ClusModel {
	...
	public MyNode cloneNode() throws ClusException {
		...
		if (m_TargetStat == null)
			...
			// reason: subtree raising in C45 pruning did not output correct nb of examples
			// in each leaf of the original
			// tree
	...
	public double checkTotalWeight() {
		if (atBottomLevel()) {
			...
			if (Math.abs(getClusteringStat().getTotalWeight() - sum) > 1e-6) {
				System.err.println(
				        "ClusNode::checkTotalWeight() error: " + getClusteringStat().getTotalWeight() + " <> " + sum);
			}
			return sum;
		}
	}
	...
	public boolean equals(Object other) {
		...
		if (m_Test != null && o.m_Test != null) { // two internal nodes: same tests and same children
			...
			return false;
		} else { // two leaves should give the at least same predictions, but this is not good
		         // enough for use in HashMap!
	...
	public int countPredictedRelevant() {
		...
		if (m_Test == null) { // is leaf
			if (getTargetStat() instanceof ClassificationStat) {
				ClassificationStat targetStat = (ClassificationStat) getTargetStat();
				for (int target = 0; target < targetStat.m_ClassCounts.length; target++) {
					if (targetStat.m_ClassCounts[target][0]
					        / targetStat.m_SumWeights[target] >= targetStat.m_Thresholds[target]) {
	...
	/**
	 * Store indices of tuples in leaf nodes. Used to calculate RForest proximities
	...
	/**
	 * Make prediction and store indices of tuples which ended in the same leaf as
	 * the given tuple in the leafTuples variable. This is used to calculate RForest
	 * proximities
	 * https://www.stat.berkeley.edu/~breiman/RandomForests/cc_home.htm#prox
	 * 
	 * @param tuple
	 *            Tuple to predict
	 * @param leafTuples
	 *            hash codes of tuples which are in the same leaf where the tuple
	 *            ended will be stored in this variable.
	 * 
	 * @throws ClusException
	 */
	...
	/*
	 * public final void reInitTargetStat(RowData subset) { if (m_TargetStat !=
	 * null) { ClusStatistic st = m_TargetStat.cloneStat(); st.reset();
	 * subset.calcTotalStatBitVector(st); m_TargetStat = st; } }
	 */
	...
	/*
	 * Returns the total number of nodes (incl leaves) in the tree rooted at this
	 * node
	 */
			...
			if (arity - delta == 2) {
				...
				if (examples != null) {
					if ((m_Alternatives != null) || (m_OppositeAlternatives != null)) {
						// in the case of alternative tests, the classification is done based on how
						// many of the total
						// tests predict left or right branch
						examples0 = examples.applyAllAlternativeTests(m_Test, m_Alternatives, m_OppositeAlternatives,
						        0);
						examples1 = examples.applyAllAlternativeTests(m_Test, m_Alternatives, m_OppositeAlternatives,
						        1);
					} else {
	...
	public void printModelToQuery(PrintWriter wrt, ClusRun cr, int starttree, int startitem, boolean exhaustive) {
		...
		if (exhaustive) {
			for (int i = 0; i < cr.getNbModels(); i++) {
				...
				// print the statitistics here (the format depend on the needs of the plsql
				// program)
				if (tree.getNbNodes() <= 1) { // we only look for the majority class in the data
					...
					// writer.println("INSERT INTO trees_charac
					// VALUES(T1,"+size+error+accuracy+constraint);
					wrt.println(mod.getModelSize() + ", " + (mod.m_TrainErr).getErrorClassif() + ", "
					        + (mod.m_TrainErr).getErrorAccuracy());
				}
				Global.inc_treecpt();
			} // end for
		} // end if
			...
			wrt.println("INSERT INTO trees_sets VALUES(" + Global.get_itemsetcpt() + ", '" + tabitem[lastmodel][0]
			        + "', " + tabexist[lastmodel][0] + ")");
			wrt.println(
			        "INSERT INTO greedy_trees VALUES(" + Global.get_treecpt() + ", " + Global.get_itemsetcpt() + ",1)");
			Global.inc_itemsetcpt();
			if (tree.getNbChildren() != 0) {
				printTreeInDatabase(wrt, tabitem[lastmodel], tabexist[lastmodel], 1, "greedy_trees");
			}
			wrt.println("INSERT INTO trees_charac VALUES(" + Global.get_treecpt() + ", " + mod.getModelSize() + ", "
			        + (mod.m_TrainErr).getErrorClassif() + ", " + (mod.m_TrainErr).getErrorAccuracy() + ", NULL)");
			Global.inc_treecpt();
		}
	}
	...
	public void printMultiLabelThresholds(PrintWriter writer, int treeIndex) {
		...
		if (thresholds != null) {
			...
			for (int i = 0; i < thresholds.length; i++) {
				writer.print(ClusFormat.FOUR_AFTER_DOT.format(thresholds[i])
				        + (i == thresholds.length - 1 ? "]" + System.lineSeparator() : ", "));
			}
		} else {
	...
	public final void printTree(PrintWriter writer, StatisticPrintInfo info, String prefix, RowData examples,
		...
		if (arity > 0) {
			...
			if (arity - delta == 2) {
				...
				// false);
				((ClusNode) getChild(YES)).printTree(writer, info, prefix + getVerticalLineText() + getSpacesYes(),
				        examples0, false);
				...
				if (hasUnknownBranch()) {
					...
					// false);
					((ClusNode) getChild(NO)).printTree(writer, info, prefix + getVerticalLineText() + getSpacesNo(),
					        examples1, false);
	...
	/*
	 * Prints for each example the path that is followed in the tree, both with node
	 * identifiers, and in boolean format (used in ICDM 2011 paper on
	 * "random forest feature induction") only binary trees are supported
	 */
	public final void printPaths(PrintWriter writer, String pathprefix, String numberprefix, RowData examples,
	        OOBSelection oob_sel, boolean testset) {
	...
	/**
	 * The iterative version of the method for printing Python code. This should be
	 * called only at the root node of the tree. Two stages:<br>
	 * <ul>
	 ...
	 * @param dict
	 */
	public final void printTreeToPythonScriptIterative(PrintWriter writer, String prefix,
		...
		while (!temp.isEmpty()) {
			...
			if (children != 0 && children != 2) {
				System.err
				        .println("Warning: not a binary tree! Python code won't work properly with high probability.");
			}
			...
			if (arity > 0) {
				...
				if (m_TargetStat != null) {
					...
					if (current.m_TargetStat instanceof ClassificationStat) {
						...
						s = ((WHTDStatistic) current.m_TargetStat).getArrayOfStatisticExtended();
					} else {
	...
	public final void printTreeToPythonTreeObject(PrintWriter writer, HashMap<String, Integer> dict,
	        String modelIdentifier) {
		// {nodeID: number of children that have not been processed yet, ... }
		...
		while (!temp.isEmpty()) {
			...
			if (current.atBottomLevel()) {
				...
				// write internal line
				writer.println(pythonTreeInternalNodeLine(current, modelIdentifier,
				        dict.get(current.m_Test.getType().getName())));
				isProcessed = true;
	...
	private static String pythonTreeInternalNodeLine(ClusNode node, String modelIdentifier, int inputAttrIndex) {
		...
		String frequenciesArg = String.format("branch_frequencies=%s",
		        Arrays.toString(node.getTest().getProportions()));
		String testArg = String.format("test=BinaryNodeTest(%d, test_function=lambda t: t%s)", inputAttrIndex,
		        node.m_Test.getPythonTestString(""));
		return String.format("%s = TreeNode(%s, %s, %s)", pythonNodeName(node, modelIdentifier), childrenArg,
		        frequenciesArg, testArg);
	}
	...
	private String pythonTreeLeafNodeLine(ClusNode leaf, String modelIdentifier) {
		...
		if (m_TargetStat instanceof RegressionStat) {
			...
			statArg = String.format("prediction_statistics=RegressionStat(%s)",
			        ((WHTDStatistic) leaf.m_TargetStat).getArrayOfStatisticExtended());
		} else {
