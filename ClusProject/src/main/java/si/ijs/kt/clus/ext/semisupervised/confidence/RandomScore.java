...
public class RandomScore extends PredictionConfidence {
    ...
    public RandomScore(ClusStatManager statManager, int type) {
        super(statManager, SSLNormalization.MinMaxNormalization, SSLAggregation.Average);
        this.type = type;
    }
        ...
        for (int i = 0; i < nb_unlabeled; i++) {
            ...
            if (tuple != null && tuple.getWeight() > 0) {
                ...
                if (type == RANDOM_GAUSSIAN) {
                    m_ConfidenceScores[i] = rnd.nextGaussian() * 0.125 + 0.5; // generate normally distribuited numbers
                                                                              // with mean 0.5 and std. dev. 0.125
