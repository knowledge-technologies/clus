...
public class ClusFeatureRanking {
    ...
    public ClusErrorList computeErrorList(ClusSchema schema, ClusStatManager mgr) {
        ...
        if (mgr.getMode() == ClusStatManager.MODE_CLASSIFY) {
            ...
            error.addError(new HierErrorMeasures(error, mgr.getHier(), sett.getHMLC().getRecallValues().getDoubleVector(), HierarchyMeasures.PooledAUPRC, false, getSettings().getOutput().isGzipOutput()));
        }
        else {
    ...
    /**
     * Comptues ranks of the features. To avoid the floating-point arithmetics issues we use approximately equal when
     * computing the ranks ...
     */
