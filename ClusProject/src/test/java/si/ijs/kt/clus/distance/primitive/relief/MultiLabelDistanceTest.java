...
public class MultiLabelDistanceTest extends BaseTestCase {
  ...
  public void test() throws IOException {
    ...
    MultiLabelDistance[] distances =
        new MultiLabelDistance[] {
          new MultiLabelDistance(
              SettingsRelief.MultilabelDistance.HammingLoss, attrs, labelProbabilities),
          new MultiLabelDistance(
              SettingsRelief.MultilabelDistance.MLAccuracy, attrs, labelProbabilities),
          new MultiLabelDistance(
              SettingsRelief.MultilabelDistance.MLFOne, attrs, labelProbabilities),
          new MultiLabelDistance(
              SettingsRelief.MultilabelDistance.SubsetAccuracy, attrs, labelProbabilities)
        };
    ArrayList<Triple<Integer, Integer, double[]>> testCases =
        new ArrayList<
            Triple<Integer, Integer, double[]>>(); // index first, index second, list of distances
    testCases.add(new Triple<Integer, Integer, double[]>(0, 0, new double[] {0.0, 0.0, 0.0, 0.0}));
    testCases.add(
        new Triple<Integer, Integer, double[]>(0, 1, new double[] {2.0 / 3.0, 1.0, 1.0, 1.0}));
    testCases.add(
        new Triple<Integer, Integer, double[]>(
            1, 2, new double[] {1.0 / 3.0, 0.5, 1.0 / 3.0, 1.0}));
    testCases.add(
        new Triple<Integer, Integer, double[]>(
            1, 3, new double[] {1.0 / 9.0, 1.0 / 6.0, 1.0 / 11.0, 1.0 / 3.0}));
    testCases.add(
        new Triple<Integer, Integer, double[]>(
            2, 3, new double[] {4.0 / 9.0, 2.0 / 3.0, 1.0 / 2.0, 1.0}));
    ...
    for (Triple<Integer, Integer, double[]> testCase : testCases) {
      ...
      for (int i = 0; i < distances.length; i++) {
        ...
        assertEquals(
            ...
            precision,
            String.format(
                "Distance %s between tuples %d: %s and %d: %s wrong",
                distances[i].toString(), ind1, t1.toString(), ind2, t2.toString()));
      }
    }
  }
