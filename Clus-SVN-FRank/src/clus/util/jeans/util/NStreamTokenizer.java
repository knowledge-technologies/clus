...
public class NStreamTokenizer {
    ...
    public void doRead(int start, int len) throws IOException {
        ...
        if (nbRead == -1)
            nbRead = 0;
        int till = start + len;
        for (int ctr = start + nbRead; ctr < till; ctr++)
            m_doubleBuffer[ctr] = CHAR_EOF;
    }
    ...
    public void pushBackChar() {
        ...
        if (m_doubleBuffer[bufPos] == CHAR_LF)
            m_lineNo--;
    }
    ...
    public String makeString() {
        ...
        if (bufPos >= m_markPos) {
            ...
            return (new String(m_doubleBuffer, bufPos, BUFFER_SINGLE - bufPos - 1)) + (new String(m_doubleBuffer, 0, m_markPos));
        }
    }
