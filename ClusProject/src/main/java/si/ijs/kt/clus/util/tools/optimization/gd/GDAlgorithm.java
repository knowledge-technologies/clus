...
public class GDAlgorithm extends OptimizationAlgorithm {
	...
	/**
	 * Includes the information about settings of optimization. Has all the helping
	...
	/**
	 * After finding the early stop, how many times we have tried to reduce the step
	 * size
	 */
	 ...
	 *                        procedure is based on this data information. Warning:
	 *                        The parameter may be modified!
	 */
	public GDAlgorithm(ClusStatManager stat_mgr, OptimizationProblem.OptimizationParameter dataInformation,
			ClusRuleSet rset) {
		super(stat_mgr);
	...
	public ArrayList<Double> optimize() {
		ClusLogger.info(
				"Gradient descent: Optimizing rule weights (" + getSettings().getRules().getOptGDMaxIter() + ") ");
		...
		if (m_GDProbl.isClassifTask()) {
			try {
				throw new ClusException(
						"Classification not yeat implemented for gradient descent. Skipping the optimization.");
			} catch (Exception s) {
				s.printStackTrace();
			}
			return null;
		...
		while (nbOfIterations < getSettings().getRules().getOptGDMaxIter()) {
			...
			if (nbOfIterations % (Math.ceil(getSettings().getRules().getOptGDMaxIter() / 50.0)) == 0) {
				ClusLogger.finer(String.format("%s %%",
						nbOfIterations * 100 / Math.ceil(getSettings().getRules().getOptGDMaxIter())));
			}
			if (nbOfIterations % m_earlyStopStep == 0) {
				...
				if (getSettings().getRules().getOptGDEarlyStopAmount() > 0 && m_GDProbl.isEarlyStop(m_weights)) {
					...
					if (!getSettings().getRules().isOptGDIsDynStepsize()
							&& m_earlyStopStepsizeReducedNb < getSettings().getRules().getOptGDNbOfStepSizeReduce()) {
			...
			for (int iiGradient = 0; iiGradient < maxGradients.length; iiGradient++) {
				...
				// c) We are changing the value of the weight. Let's compute the covariance if
				// it is not yet done
				...
				// For detecting oscillation. If oscillation is already true, do not change it
				// to false.
			...
			// It should be smaller because otherwise we are going even further from the
			// optimal point.
			if (oscillation && !getSettings().getRules().isOptGDIsDynStepsize()) {
				...
				if (debugPrint) {
					ClusLogger.info("DEBUG: Detected oscillation on iteration " + nbOfIterations
							+ ", reducing step size of: " + m_GDProbl.m_stepSize);
				}
				// System.exit(1); //DEBUG
				// For MaxLoss combination the oscillation may be because we are finding some
				// local
				// optimum point for some weight and it is the most significant. Let us put this
				// weight
				// to a banned list for some iterations
				if (nbOfIterations > 10 && (getSettings().getRules().getOptGDMTGradientCombine()
						.equals(OptimizationGDMTCombineGradient.MaxLoss)
						|| getSettings().getRules().getOptGDMTGradientCombine()
								.equals(OptimizationGDMTCombineGradient.MaxLossFast))) {
					putOscillatingWeightsToBan(nbOfIterations);
				} else {
					// This is needed if we are changing more than one dimension at time.
					// The step size is made for the biggest oscillation and thus some of the other
					// oscillating ones
			...
			for (int iiGradient = 0; iiGradient < maxGradients.length; iiGradient++) {
				m_weights.set(maxGradients[iiGradient],
						m_weights.get(maxGradients[iiGradient]).doubleValue() + valueChange[iiGradient]);
			}
	...
	/**
	 * Puts oscillating weights to ban, so that we can continue on the rest of
	 * weights. This is used only if for maxLoss combining something happens.
	 */
	...
	private void reduceStepSizeDueOscillation() {
		...
		// We make the new step size a little smaller than is limit (because of rounding
		// mistakes)
	...
	/**
	 * Information about the previous change for oscillation warning. The change
	 * value
	 */
	...
	/**
	 * Information about the previous change for oscillation warning. The changed
	 * dimension.
	 */
	...
	/**
	 * A iteration has been done and oscillation not detected - store the previous
	 * changes
	 */
	...
	/**
	 * Reverse previous (not this) step if oscillation found. Thus the state should
	 * be like it was before. This has to be called before storeTheOscillationData.
	 */
	private void reversePreviousStep() {
		...
		for (int iiGradient = 0; iiGradient < m_iPrevDimension.length; iiGradient++) {
			m_weights.set(m_iPrevDimension[iiGradient],
					m_weights.get(m_iPrevDimension[iiGradient]).doubleValue() - m_prevChange[iiGradient]);
		}

		// Compute the gradients again. This should no be done so often that this costs
		// very much. Thus we
	...
	/**
	 * Stores the used gradients for oscillation detection later. Also
	 * initialization tasks for oscillation.
	 */
	...
	/**
	 * Oscillation is detected if same dimension is changed such that the direction
	 * is different and the new step is bigger than previous.
	 * 
	...
	private boolean detectOscillation(int iiNewChange, double valueChange) {
		...
		// Go through the previous changes and check if some of them is for same
		// dimension
		for (int iiPrevChange = 0; m_prevChange != null && iiPrevChange < m_prevChange.length; iiPrevChange++) {
			...
			if (m_iPrevDimension[iiPrevChange] == m_iNewDimension[iiNewChange]) {
				...
				if (valueChange * m_prevChange[iiPrevChange] < 0
					...
					// System.err.println("DEBUG: Found out oscillation for weight " +
					// m_iPrevDimension[iiPrevChange] +
					// " that was nb " +iiPrevChange + " last iteration and "+ iiNewChange + "
					// now.");
					...
					// + m_iPrevDimension[iiPrevChange]
					// + " was changed consequently with change values: " +
					// m_prevChange[iiPrevChange]
					// + " and " + valueChange + ".\n");

					// How much we need step size reduction
					double needReduction = Math.abs(m_prevChange[iiPrevChange]) / Math.abs(valueChange);

					// If the reduction is greater than any of the previous on this iteration, do
					// it.
	...
	public void OutputLog(int iterNro, PrintWriter wrt) {
		...
		double trainingFitness = m_GDProbl.calcFitness(m_weights, m_GDProbl.m_ruleSet);
		double testFitness = 0;
		if (getSettings().getRules().getOptGDEarlyStopAmount() > 0)
			testFitness = m_GDProbl.m_earlyStopProbl.calcFitness(m_weights, m_GDProbl.m_ruleSet);
