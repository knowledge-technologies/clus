...
public class ClusStatManager implements Serializable {
	...
	public void initWeights(ClusNormalizedAttributeWeights result, NumericAttrType[] num, NominalAttrType[] nom,
		...
		if (winfo.hasArrayIndexNames()) {
			...
			if (getSettings().getHMLC().isHierAndClassAndReg()) {
				...
				if (l + m_Hier.getTotal() != nbattr) {
					throw new ClusException("Number of attributes is " + (nbattr - m_Hier.getTotal())
							+ " but weight vector has only " + l + " components");
				}
				...
				if (nbattr != winfo.getVectorLength()) {
					throw new ClusException("Number of attributes is " + nbattr + " but weight vector has only "
	...
	public void initClusteringWeights() throws ClusException {
		...
		initWeights((ClusNormalizedAttributeWeights) m_ClusteringWeights, num, nom,
				getSettings().getAttribute().getClusteringWeights());
	...
	public void check() throws ClusException {
		...
		if (getSettings().getBeamSearch().isBeamSearchMode()
				&& (getSettings().getBeamSearch().getBeamSimilarity() != 0.0)) {
		...
		if (nb_types > 1) {
			if (!getSettings().getRelief().isRelief()) {
	...
	public void initStructure() throws IOException {
		...
		case MODE_HIERARCHICAL:
			...
			break;
		case MODE_SSPD:
			m_SSPDMtrx = SSPDMatrix.read(
					getSettings().getGeneric().getFileAbsolute(getSettings().getGeneric().getAppName() + ".dist"),
					getSettings());
			break;
		}
	}
	...
	public boolean heuristicNeedsCombStat() {
		if (isRuleInduceOnly()) {
			return Arrays
					.asList(Heuristic.Default, Heuristic.DispersionAdt, Heuristic.DispersionMlt,
							Heuristic.RDispersionAdt, Heuristic.RDispersionMlt)
					.contains(getSettings().getTree().getHeuristic());
		} else {
			return false;
		}
	}
	...
	public synchronized void initStatistic() throws ClusException {
		...
		case MODE_HIERARCHICAL:
			...
			if (getSettings().getHMLC().isHierAndClassAndReg()) {
				...
				m_StatisticAttrUse[AttributeUseType.All.getIndex()] = new CombStatClassRegHier(this, num3, nom3,
						(WHTDStatistic) clustering.cloneStat());
			} else {
		...
		case MODE_SSPD:
			...
			setClusteringStatistic(
					new SumPairwiseDistancesStat(getSettings(), m_SSPDMtrx, TimeSeriesPrototypeComplexity.NPairs));
			setTargetStatistic(
					new SumPairwiseDistancesStat(getSettings(), m_SSPDMtrx, TimeSeriesPrototypeComplexity.NPairs));
			break;
	...
	public void initBeamSearchHeuristic() throws ClusException {
		if (getSettings().getTree().getHeuristic().equals(Heuristic.ReducedError)) {
			...
			m_Heuristic = new ClusBeamHeuristicMEstimate(createClusteringStat(),
					getSettings().getNominal().getMEstimate(), getSettings());
	...
	public void initHeuristic() throws ClusException {
		...
		if (m_Mode == MODE_HIERARCHICAL) {
			...
			if (getSettings().getTree().getHeuristic().equals(Heuristic.VarianceReductionGIS)) {
				m_Heuristic = new GISHeuristic(getClusteringWeights(),
		...
		if (getSettings().getTree().getHeuristic().equals(Heuristic.VarianceReductionMissing)) {
			m_Heuristic = new VarianceReductionHeuristicInclMissingValues(getClusteringWeights(),
					m_Schema.getAllAttrUse(AttributeUseType.Clustering), createClusteringStat(), getSettings());
			return;
		}
		if (num.length > 0 && nom.length > 0) {
			if (!getSettings().getTree().getHeuristic().equals(Heuristic.Default)
					&& !getSettings().getTree().getHeuristic().equals(Heuristic.VarianceReduction)) {
				throw new ClusException(
						"Only SS-Reduction heuristic can be used for combined classification/regression trees!");
			}
			m_Heuristic = new VarianceReductionHeuristicEfficient(getClusteringWeights(),
					m_Schema.getAllAttrUse(AttributeUseType.Clustering), getSettings());
			getSettings().getTree().setHeuristic(Heuristic.VarianceReduction);
		} else if (num.length > 0) {
			ArrayList<Heuristic> allowedHeuristics = new ArrayList<>();
			allowedHeuristics.add(Heuristic.Default);
			allowedHeuristics.add(Heuristic.VarianceReduction);
			...
			if (!allowedHeuristics.contains(getSettings().getTree().getHeuristic())) {
				throw new ClusException(
						"Only SS-Reduction heuristic and GISHeuristic can be used for regression trees!"); // daniela
																											// partially
			} else if (getSettings().getTree().getHeuristic().equals(Heuristic.VarianceReductionGIS)) {
				m_Heuristic = new GISHeuristic(getClusteringWeights(),
						m_Schema.getNumericAttrUse(AttributeUseType.Clustering), getSettings()); // daniela
			...
			if (getSettings().getTree().getHeuristic().equals(Heuristic.SemiSupervised)) {
				...
				m_Heuristic = new GISHeuristic(getClusteringWeights(),
						m_Schema.getNominalAttrUse(AttributeUseType.Clustering), getSettings());
	...
	public ClusErrorList createErrorMeasure(MultiScore score) throws ClusException {
		...
		case MODE_HIERARCHICAL_MTR:
			...
			if (getSettings().getAttribute().hasNonTrivialWeights()) {
				parent.addError(new si.ijs.kt.clus.error.RMSError(parent, numHMTR, m_NormalizationWeights,
						SettingsHMTR.HMTR_ERROR_POSTFIX));
			}
			parent.addError(
					new si.ijs.kt.clus.error.PearsonCorrelation(parent, numHMTR, SettingsHMTR.HMTR_ERROR_POSTFIX));
			break;
		case MODE_ILEVELC:
	...
	public PruneTree getTreePrunerNoVSB() throws ClusException {
		...
		if (m_Mode == MODE_REGRESSION || m_Mode == MODE_HIERARCHICAL_MTR) {
			...
			if (sett.getPruningMethod().equals(PruningMethod.M5Multi)) {
				return new M5PrunerMulti(getClusteringWeights(), mult);
			}
			if (sett.getPruningMethod().equals(PruningMethod.Default)
					|| sett.getPruningMethod().equals(PruningMethod.M5)) {
				sett.setPruningMethod(PruningMethod.M5);
				return new M5Pruner(getClusteringWeights(), mult);
			}
		} else if (m_Mode == MODE_CLASSIFY) {
			if (sett.getPruningMethod().equals(PruningMethod.Default)
					|| sett.getPruningMethod().equals(PruningMethod.C45)) {
			...
			if (sett.getPruningMethod().equals(PruningMethod.M5)) {
				return new M5Pruner(getClusteringWeights(), sett.getM5PruningMult());
			}
		} else if (m_Mode == MODE_HIERARCHICAL) {
			if (sett.getPruningMethod().equals(PruningMethod.M5)) {
				double mult = sett.getM5PruningMult();
			...
			if (sett.getPruningMethod().equals(PruningMethod.EncodingCost)) {
			...
			if (sett.getPruningMethod().equals(PruningMethod.C45)) {
				return new C45Pruner();
			}
		}
	...
	public void initRuleSettings() throws ClusException {
		...
		if (!Arrays.asList(Heuristic.DispersionAdt, Heuristic.DispersionMlt, Heuristic.RDispersionAdt,
				Heuristic.RDispersionMlt).contains(sett.getHeuristic()) && setr.isHeurRuleDist()) {
		...
		if (setr.isRandomRules()) {
			...
			case HeurOnly:
				if ((prediction.equals(RulePredictionMethod.DecisionList))
						|| (prediction.equals(RulePredictionMethod.Union))) {
					setr.setRulePredictionMethod(RulePredictionMethod.CoverageWeighted);
				}
				setr.setCoveringWeight(0.0);
				if (setr.getHeurRuleDistPar() < 0) {
					throw new ClusException("Clus heuristic covering: HeurRuleDistPar must be >= 0!");
				}

				if (!Arrays.asList(Heuristic.DispersionAdt, Heuristic.DispersionMlt, Heuristic.RDispersionAdt,
						Heuristic.RDispersionMlt).contains(sett.getHeuristic())) {
